...
import { extractChannelValues, isFalsyValue, calculateMultiAxisChannelDomains, calculateAllIndependentScaleInfo } from './utils';
import { extractSingleAxisScaleInfo, extractMultiAxisScaleInfo, processSingleAxisFiltering, processMultiAxisViewFiltering, processMultiAxisMarkFiltering, updateChannelDomains } from './adaptiveFilter';
...
function calculatePointInset(view) {
  ...
  for (const [mark, state] of view.markState.entries()) {
    ...
    if (((_d = sizeChannel.values) === null || _d === void 0 ? void 0 : _d.length) > 0) {
      const sizes = sizeChannel.values.filter(item => item.value !== undefined).flatMap(item => Array.isArray(item.value) ? item.value : [item.value]).filter(value => typeof value === 'number' && !isNaN(value));
      if (sizes.length > 0) {
...
function processMultiAxisFiltering({
  ...
  if (isViewSlider) {
    ...
    const multiAxisScaleInfo = extractMultiAxisScaleInfo(shouldFilterXAxis, scale, scaleX, scaleY, channelDomain);
...
function createValueChangeHandler({
  ...
  return throttle(event => __awaiter(this, void 0, void 0, function* () {
    const {
      initValue = false
    } = event;
