...
import { BREAK_GROUP_CLASS_NAME, PLOT_CLASS_NAME } from '../../runtime/constant';
...
export const AxisBreaks = (_, params) => {
  ...
  const resetCollapsed = () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    setState('options', prev => {
      ...
      const newMarks = marks.map(mark => {
        const breaks = get(mark, 'scale.y.breaks', []);
        set(mark, 'scale.y.breaks', breaks.map(b => Object.assign(Object.assign({}, b), {
          collapsed: false
        })));
        return mark;
      });
  ...
  return option => {
    const {
      ...
      style = __rest(option, ["key", "start", "end", "gap", "vertices", "lineWidth", "verticeOffset"]);
    ...
    for (const [boundaryIndex, {
      ...
      if (boundaryIndex === 0) {
        ...
        linePath += `L ${plotWidth - PADDING},${y} L ${[...pathPoints].reverse().join(' L ')} L ${PADDING},${y} Z`;
