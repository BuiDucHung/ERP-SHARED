...
const MaybeDefaultY = () => {
  return (I, mark) => {
    ...
    if (Array.isArray(data) && (data.every(Array.isArray) || !data.some(helper_1.isObject))) {
      const extractY = (data, index) => Array.isArray(data[0]) ? data.map(item => item[index]) : [data[index]];
