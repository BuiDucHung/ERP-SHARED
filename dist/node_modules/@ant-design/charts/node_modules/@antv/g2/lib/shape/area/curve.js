...
const DoubleArea = (0, createElement_1.createElement)(g => {
  ...
  (0, selection_1.select)(g).maybeAppend('connect-path', () => document.createElement('path', {})).style('d', connectPath).call(utils_1.applyStyle, connectStyle);
  (0, selection_1.select)(g).maybeAppend('area-path', () => document.createElement('path', {})).style('d', areaPath).call(utils_1.applyStyle, areaStyle);
});
const Curve = (options, context) => {
  ...
  return (P, value, defaults) => {
    ...
    const fill = gradient && sc ? (0, utils_1.computeGradient)(sc, sx, sy, gradient, undefined, tpShape) : color;
    ...
    const getPathNode = path => {
      return (0, selection_1.select)(document.createElement('path', {})).style('d', path || '').call(utils_1.applyStyle, finalStyle).node();
    };
    if (!(0, coordinate_1.isPolar)(coordinate)) {
      ...
      const areaPath = points => {
        ...
        return tpShape ? (0, d3_shape_1.area)().y((_, idx) => Y1[idx][1]).x1((_, idx) => Y1[idx][0]).x0((_, idx) => Y0[idx][0]).defined((_, idx) => [...Y1[idx], ...Y0[idx]].every(defined)).curve(curve)(Y1) : (0, d3_shape_1.area)().x((_, idx) => Y1[idx][0]).y1((_, idx) => Y1[idx][1]).y0((_, idx) => Y0[idx][1]).defined((_, idx) => [...Y1[idx], ...Y0[idx]].every(defined)).curve(curve)(Y1);
      };
      ...
      return (0, selection_1.select)(new DoubleArea()).style('areaStyle', finalStyle).style('connectStyle', Object.assign(Object.assign({}, connectStyle), style)).style('areaPath', areaPath(P)).style('connectPath', MS.map(areaPath).join('')).node();
    } else {
      /**
      ...
      const areaRadialPath = points => {
        ...
        const Y1 = points.slice(0, points.length / 2);
        const Y0 = points.slice(points.length / 2);
        return (0, d3_shape_1.areaRadial)().angle((_, idx) => (0, vector_1.angleWithQuadrant)((0, vector_1.sub)(Y1[idx], center))).outerRadius((_, idx) => (0, vector_1.dist)(Y1[idx], center)).innerRadius((_, idx) => (0, vector_1.dist)(Y0[idx], center)).defined((_, idx) => [...Y1[idx], ...Y0[idx]].every(defined)).curve(curve)(Y0);
      };
      // Draw one area of connected defined points.
      if (!missing || connectNulls && !Object.keys(connectStyle).length) {
      ...
      // One for unconnected defined points.
      // One for connected segments.
      return (0, selection_1.select)(new DoubleArea()).style('areaStyle', finalStyle).style('connectStyle', Object.assign(Object.assign({}, connectStyle), style)).style('areaPath', areaRadialPath(P)).style('connectPath', MS.map(areaRadialPath).join('')).node();
    }
  };
};
exports.Curve = Curve;
