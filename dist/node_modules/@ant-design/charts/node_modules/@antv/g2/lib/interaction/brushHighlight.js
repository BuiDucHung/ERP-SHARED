...
const ResizableMask = (0, createElement_1.createElement)(g => {
  ...
  const _b = (0, helper_1.subObject)((0, helper_1.omitPrefixObject)(style, 'handleNW', 'handleNE'), 'handleN'),
    {
      render: handleNRender = renderRect
    } = _b,
    handleNStyle = __rest(_b, ["render"]);
  const _c = (0, helper_1.subObject)(style, 'handleE'),
    {
      render: handleERender = renderRect
    } = _c,
    handleEStyle = __rest(_c, ["render"]);
  const _d = (0, helper_1.subObject)((0, helper_1.omitPrefixObject)(style, 'handleSE', 'handleSW'), 'handleS'),
    {
      render: handleSRender = renderRect
    } = _d,
    handleSStyle = __rest(_d, ["render"]);
  const _e = (0, helper_1.subObject)(style, 'handleW'),
    {
      render: handleWRender = renderRect
    } = _e,
    handleWStyle = __rest(_e, ["render"]);
  const _f = (0, helper_1.subObject)(style, 'handleNW'),
    {
      render: handleNWRender = renderRect
    } = _f,
    handleNWStyle = __rest(_f, ["render"]);
  const _g = (0, helper_1.subObject)(style, 'handleNE'),
    {
      render: handleNERender = renderRect
    } = _g,
    handleNEStyle = __rest(_g, ["render"]);
  const _h = (0, helper_1.subObject)(style, 'handleSE'),
    {
      render: handleSERender = renderRect
    } = _h,
    handleSEStyle = __rest(_h, ["render"]);
  const _j = (0, helper_1.subObject)(style, 'handleSW'),
    {
  ...
  const container = (0, selection_1.select)(g).attr('className', className).style('transform', `translate(${x}, ${y})`).style('draggable', true);
  container.maybeAppend('selection', 'rect').style('draggable', true).style('fill', 'transparent').call(applyStyle, Object.assign(Object.assign({
    width,
    height
  }, (0, helper_1.omitPrefixObject)(style, 'handle')), {
    transform: undefined
  }));
...
function brush(root, _a) {
  ...
  const pointermove = event => {
    ...
    if (!mask || !isMask(target) || creating) (0, utils_1.setCursor)(root, 'crosshair');else if (isSelection(target)) (0, utils_1.setCursor)(root, 'move');else if (isHandle(target)) (0, utils_1.setCursor)(root, handles[target.id].cursor);
  };
  const pointerleave = () => {
...
function brushHighlight(root, _a) {
  ...
  const {
    setState,
    removeState
  } = (0, utils_1.useState)(state, valueof);
  ...
  return (target, viewInstances, emitter) => {
    ...
    if (facet) {
      ...
      return brushHighlight(plotArea.parentNode.parentNode, Object.assign(Object.assign({
        elements: () => (0, utils_1.selectFacetG2Elements)(target, viewInstances),
        datum: (0, utils_1.createDatumof)((0, utils_1.selectFacetViews)(target, viewInstances).map(d => d.view)),
        brushRegion: (x, y, x1, y1) => [x, y, x1, y1],
        extent: [x, y, x1, y1],
        state: (0, utils_1.mergeState)((0, utils_1.selectFacetViews)(target, viewInstances).map(d => d.options), defaultStates),
        emitter,
        scale,
        coordinate,
    ...
    const brush = brushHighlight(plotArea, Object.assign(Object.assign({
      elements: utils_1.selectG2Elements,
      key: element => element.__data__.key,
      siblings: () => selectSiblingContainers(target, viewInstances, brushKey),
      datum: (0, utils_1.createDatumof)([view, ...selectSiblingViews(target, viewInstances, brushKey).map(d => d.view)]),
      brushRegion: (x, y, x1, y1) => [x, y, x1, y1],
      extent: undefined,
      state: (0, utils_1.mergeState)([options, ...selectSiblingOptions(target, viewInstances, brushKey)], defaultStates),
      emitter,
      scale,
      coordinate,
      selectedHandles: undefined
