...
function normalizeSample(strategy) {
  ...
  if (strategy === 'lttb') return lttb_1.lttb;
  const strategies = {
    first: f => [f[0]],
    last: f => [f[f.length - 1]],
    min: (f, X, Y) => [f[(0, d3_array_1.minIndex)(f, i => Y[i])]],
    max: (f, X, Y) => [f[(0, d3_array_1.maxIndex)(f, i => Y[i])]],
    median: (f, X, Y) => [f[(0, d3_array_1.medianIndex)(f, i => Y[i])]]
  };
