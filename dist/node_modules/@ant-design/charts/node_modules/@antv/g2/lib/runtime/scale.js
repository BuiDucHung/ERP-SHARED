...
function groupTransform(markState, uidScale) {
  ...
  const scaleGroups = (0, d3_array_1.rollups)(channels, channels => channels.map(d => uidScale.get(d.scale.uid)), d => d.name).filter(([, scales]) => scales.some(d => typeof d.getOptions().groupTransform === 'function') &&
    ...
    for (const channel of channels) {
      ...
      if (scale && !independent) {
        ...
        const options1 = Object.assign(Object.assign({}, staticScale), {
          ...
          name: channel,
          guide: (0, util_1.omit)(component, NONE_STATIC_KEYS)
        });
...
function useRelation(relations) {
  if (!relations || !Array.isArray(relations)) return [helper_1.identity, helper_1.identity];
...
function syncFacetsScaleByChannel(scales, channel) {
  ...
  const syncedD = S.every(isQuantitativeScale) ? (0, d3_array_1.extent)(D) : S.every(isDiscreteScale) ? Array.from(new Set(D)) : null;
...
function inferOptionsQ(coordinates, options) {
  const {
    interpolate = scale_1.createInterpolateValue,
    nice = false,
    tickCount = 5
  } = options;
...
function inferPadding(type, name, coordinates) {
  ...
  if (type === 'band') return (0, coordinate_1.isTheta)(coordinates) ? 0 : 0.1;
...
function inferRangeQ(name, palette) {
  ...
  if (name === 'color') return [(0, array_1.firstOf)(palette), (0, array_1.lastOf)(palette)];
