...
function inferLegendComponentType(scales, coordinates) {
  ...
  const fieldScales = (0, d3_array_1.groups)(nonConstants, groupKey).map(([key, scales]) => [key, [...scales, ...constants]]).filter(([, scales]) => scales.some(scale => scale.type !== 'constant'));
...
function getScaleType(scale) {
  ...
  if (type in scale_2.ContinuousScale) return 'continuous';
  if (type in scale_2.DiscreteScale) return 'discrete';
  if (type in scale_2.DistributionScale) return 'distribution';
  if (type in scale_2.ConstantScale) return 'constant';
  return null;
}
function inferAxisComponentType(scales, coordinates) {
  return scales.map(scale => {
    ...
    if ((0, coordinate_2.isHelix)(coordinates) || (0, coordinate_2.isTheta)(coordinates)) return null;
    if ((0, coordinate_2.isTranspose)(coordinates) && ((0, coordinate_2.isPolar)(coordinates) || (0, coordinate_2.isRadial)(coordinates))) return null;
    // infer axis
    if (name.startsWith('x')) {
      if ((0, coordinate_2.isPolar)(coordinates)) return ['axisArc', [scale]];
      if ((0, coordinate_2.isRadial)(coordinates)) return ['axisLinear', [scale]];
      return [(0, coordinate_2.isTranspose)(coordinates) ? 'axisY' : 'axisX', [scale]];
    }
    if (name.startsWith('y')) {
      if ((0, coordinate_2.isPolar)(coordinates)) return ['axisLinear', [scale]];
      if ((0, coordinate_2.isRadial)(coordinates)) return ['axisArc', [scale]];
    ...
    if (name.startsWith('position')) {
      if ((0, coordinate_2.isRadar)(coordinates)) return ['axisRadar', [scale]];
      if (!(0, coordinate_2.isPolar)(coordinates)) return ['axisY', [scale]];
    }
    return null;
  }).filter(helper_1.defined);
}
...
function angleOf(coordinates) {
  ...
  if (polar.length) {
    ...
    const {
      startAngle,
      endAngle
    } = (0, coordinate_1.getPolarOptions)(lastPolar);
  ...
  if (radial.length) {
    ...
    const {
      startAngle,
      endAngle
    } = (0, coordinate_1.getRadialOptions)(lastRadial);
    return [startAngle, endAngle];
  }
...
function inferAxisPositionAndOrientation(type, ordinalPosition, relativeScales, scales, coordinates) {
  ...
  if (type === 'axisRadar') {
    ...
    const positionLength = (0, coordinate_2.isRadar)(coordinates) ? positions.length : positions.length - 1;
    const angle = (endAngle - startAngle) / positionLength * index + startAngle;
    return ['center', angle];
  }
  if (type === 'axisY' && (0, coordinate_2.isParallel)(coordinates)) {
    return (0, coordinate_2.isTranspose)(coordinates) ? ['center', 'horizontal'] : ['center', 'vertical'];
  }
  ...
  if ((0, coordinate_2.isPolar)(coordinates)) return ['center', null];
  if ((0, coordinate_2.isRadial)(coordinates)) return ['center', null];
  if (type === 'axisX' && (0, coordinate_2.isReflect)(coordinates) || type === 'axisX' && (0, coordinate_2.isReflectY)(coordinates)) {
    return ['top', null];
  }
...
function inferComponentPositionAndOrientation(type, defaultPosition, defaultOrientation, guide, relativeScales, scales, coordinates) {
  ...
  if (typeof type === 'string' && type.startsWith('legend') && (0, coordinate_2.isPolar)(coordinates)) {
...
function inferScrollableType(name, type, coordinates = []) {
  if (name === 'x') return (0, coordinate_2.isTranspose)(coordinates) ? `${type}Y` : `${type}X`;
  if (name === 'y') return (0, coordinate_2.isTranspose)(coordinates) ? `${type}X` : `${type}Y`;
  return null;
}
/**
...
function computeCategoryLegendSize(component, crossSize0, crossPadding, position, theme, library) {
  ...
  const _a = styleOf(),
    ...
    rest = __rest(_a, ["focus", "itemSpacing", "focusMarkerSize", "itemMarkerSize", "titleSpacing", "rowPadding", "colPadding", "maxCols", "maxRows"]);
  const {
    cols,
    length
  } = component;
  ...
  if (render && typeof document !== 'undefined') {
    ...
    const formatLabel = d => {
      ...
      return typeof labelFormatter === 'string' ? (0, d3_format_1.format)(labelFormatter)(d) : labelFormatter(d);
    };
    ...
    if (width) style.width = `${width}px`;else if (!isVertical) style.width = `${crossSize}px`;
    if (height) style.height = `${height}px`;else if (isVertical) style.height = `${crossSize}px`;
...
function styleOf(axis, position, theme) {
  ...
  const {
    axis: baseStyle,
    // @ts-ignore
    [`axis${(0, helper_1.capitalizeFirst)(position)}`]: positionStyle
  } = theme;
  return (0, util_1.deepMix)({
    title: defaultTitle
...
function labelsOf(scale, labelFormatter, tickFilter) {
  ...
  const formatter = labelFormatter ? typeof labelFormatter === 'string' ? (0, d3_format_1.format)(labelFormatter) : labelFormatter : scale.getFormatter ? scale.getFormatter() : d => `${d}`;
...
function normalizeLabel(d) {
  if (d instanceof g_1.DisplayObject) return d;
  return new g_1.Text({
    style: {
      text: `${d}`
    }
