...
function getFunnelPoints(points, nextPoints, previousPoints, coordinate, reverse) {
  ...
  if ((0, coordinate_1.isTranspose)(coordinate)) {
    if (reverse) {
      const newP0 = [previousPoints ? previousPoints[1][0] : p0[0], p0[1]];
      const newP3 = [previousPoints ? previousPoints[2][0] : p3[0], p3[1]];
  ...
  if (reverse) {
    const newP0 = [p0[0], previousPoints ? previousPoints[1][1] : p0[1]];
    const newP3 = [p3[0], previousPoints ? previousPoints[2][1] : p3[1]];
    return [newP0, p1, p2, newP3];
  }
...
function createRoundedPath(points, radius) {
  ...
  const rs = isLongBaseOnTop ? [getRadiusOfLongerBase(angle, r0), getRadiusOfLongerBase(angle, r1), r2, r3] : [r0, r1, getRadiusOfLongerBase(angle, r2), getRadiusOfLongerBase(angle, r3)];
  ...
  return [`M${p0[0] + rs[0]} ${p0[1]}`, `L${p1[0] - rs[1]} ${p1[1]}`, `Q${p1[0]} ${p1[1]} ${p1[0] - ofs[1].x} ${p1[1] + ofs[1].y}`, `L${p2[0] + ofs[2].x} ${p2[1] - ofs[2].y}`, `Q${p2[0]} ${p2[1]} ${p2[0] - rs[2]} ${p2[1]}`, `L${p3[0] + rs[3]} ${p3[1]}`, `Q${p3[0]} ${p3[1]} ${p3[0] - ofs[3].x} ${p3[1] - ofs[3].y}`, `L${p0[0] + ofs[0].x} ${p0[1] + ofs[0].y}`, `Q${p0[0]} ${p0[1]} ${p0[0] + rs[0]} ${p0[1]}`, 'Z'].join(' ');
}
/**
...
const Funnel = (options, context) => {
  const {
    ...
    style = __rest(options, ["adjustPoints", "radius", "radiusTopLeft", "radiusTopRight", "radiusBottomRight", "radiusBottomLeft", "innerRadius", "innerRadiusTopLeft", "innerRadiusTopRight", "innerRadiusBottomRight", "innerRadiusBottomLeft", "first", "last"]);
  const {
    coordinate,
    document
  } = context;
  return (points, value, defaults, point2d) => {
    ...
    const standardDirRadius = [first ? radiusTopLeft !== null && radiusTopLeft !== void 0 ? radiusTopLeft : innerRadiusTopLeft : innerRadiusTopLeft, first ? radiusTopRight !== null && radiusTopRight !== void 0 ? radiusTopRight : innerRadiusTopRight : innerRadiusTopRight, last ? radiusBottomRight !== null && radiusBottomRight !== void 0 ? radiusBottomRight : innerRadiusBottomRight : innerRadiusBottomRight, last ? radiusBottomLeft !== null && radiusBottomLeft !== void 0 ? radiusBottomLeft : innerRadiusBottomLeft : innerRadiusBottomLeft];
    const path = standardDirRadius.find(r => r > 0) ? createRoundedPath([p0, p1, p2, p3], standardDirRadius) : (0, d3_shape_1.line)().curve(d3_shape_1.curveLinearClosed)([p0, p1, p2, p3]);
    return (0, selection_1.select)(document.createElement('path', {})).call(utils_1.applyStyle, rest).style('d', path).style('fill', color).style('fillOpacity', opacity).call(utils_1.applyStyle, style).node();
  };
};
