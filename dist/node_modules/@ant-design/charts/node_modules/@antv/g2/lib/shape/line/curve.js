...
const DoublePath = (0, createElement_1.createElement)(g => {
  ...
  (0, selection_1.select)(g).maybeAppend('line', () => document.createElement('path', {})).style('d', d1).call(utils_1.applyStyle, style1);
  (0, selection_1.select)(g).maybeAppend('line1', () => document.createElement('path', {})).style('d', d2).call(utils_1.applyStyle, style2);
});
/**
...
const Curve = (options, context) => {
  ...
  return (P, value, defaults) => {
    ...
    const stroke = gradient && sc ? (0, utils_1.computeGradient)(sc, sx, sy, gradient, gradientColor, tpShape) : color;
    ...
    if ((0, coordinate_1.isPolar)(coordinate)) {
      ...
      linePath = points => (0, d3_shape_1.lineRadial)().angle((_, idx) => (0, vector_1.angleWithQuadrant)((0, vector_1.sub)(points[idx], center))).radius((_, idx) => (0, vector_1.dist)(points[idx], center)).defined(([x, y]) => defined(x) && defined(y)).curve(curve)(points);
    } else {
      linePath = (0, d3_shape_1.line)().x(d => d[0]).y(d => d[1]).defined(([x, y]) => defined(x) && defined(y)).curve(curve);
    }
    ...
    if (!missing || connectNulls && !Object.keys(connectStyle).length) {
      return (0, selection_1.select)(document.createElement('path', {})).style('d', linePath(DP) || []).call(utils_1.applyStyle, finalStyle).node();
    }
    // Draw one path of unconnected defined points.
    if (missing && !connectNulls) {
      return (0, selection_1.select)(document.createElement('path', {})).style('d', linePath(P)).call(utils_1.applyStyle, finalStyle).node();
    }
    // Draw two path.
    // One for unconnected defined points.
    // One for connected segments.
    const connectPath = segments => segments.map(linePath).join(',');
    return (0, selection_1.select)(new DoublePath()).style('style1', Object.assign(Object.assign({}, finalStyle), connectStyle)).style('style2', finalStyle).style('d1', connectPath(MS)).style('d2', linePath(P)).node();
  };
};
exports.Curve = Curve;
exports.Curve.props = {
