...
const BadgeShape = (0, createElement_1.createElement)(g => {
  ...
  const bgShape = (0, selection_1.select)(g).maybeAppend('marker', () => new component_1.Marker({})).call(selection => selection.node().update(Object.assign({
    symbol
  }, markerStyle))).node();
  const [x, y] = inferTextPosition(bgShape);
  (0, selection_1.select)(g).maybeAppend('text', 'text').style('x', x).style('y', y).call(utils_1.applyStyle, rest);
});
const Badge = (options, context) => {
  ...
  return (points, value, defaults) => {
    ...
    const [[x0, y0]] = points;
    return (0, selection_1.select)(new BadgeShape()).call(utils_1.applyStyle, rest).style('transform', `translate(${x0},${y0})`).call(utils_1.applyStyle, textStyle).call(utils_1.applyStyle, style).node();
  };
};
exports.Badge = Badge;
exports.Badge.props = {
