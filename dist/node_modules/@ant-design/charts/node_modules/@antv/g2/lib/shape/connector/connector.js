...
function inferConnectorPath(points) {
  return (0, d3_shape_1.line)().x(d => d[0]).y(d => d[1])(points);
}
...
const Connector = (options, context) => {
  ...
  return (points, value, defaults) => {
    ...
    return (0, selection_1.select)(new g_1.Path()).call(utils_1.applyStyle, rest).style('d', inferConnectorPath(P)).style('stroke', color || defaultColor).style('transform', transform).style('markerEnd', endMarker ? new component_1.Marker({
      className: 'marker',
      style: Object.assign(Object.assign({}, makerStyle), {
        symbol: inferSymbol
      })
    }) : null).call(utils_1.applyStyle, style).node();
  };
};
