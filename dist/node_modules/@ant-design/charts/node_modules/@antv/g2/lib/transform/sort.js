...
function createReducer(channel, options, encode) {
  ...
  if (reducer === 'max') return GI => (0, d3_array_1.max)(GI, i => +V[i]);
  if (reducer === 'min') return GI => (0, d3_array_1.min)(GI, i => +V[i]);
  if (reducer === 'sum') return GI => (0, d3_array_1.sum)(GI, i => +V[i]);
  if (reducer === 'median') return GI => (0, d3_array_1.median)(GI, i => +V[i]);
  if (reducer === 'mean') return GI => (0, d3_array_1.mean)(GI, i => +V[i]);
...
const Sort = (options = {}) => {
  return (I, mark) => {
    ...
    return [sortedI, (0, util_1.deepMix)(mark, {
      scale: {
        [channel]: {
          domain: slicedDomain
        }
