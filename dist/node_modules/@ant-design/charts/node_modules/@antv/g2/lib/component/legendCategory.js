...
function inferItemMarker(options, context) {
  ...
  const create = (name, d) => {
    ...
    return () => (0, marker_1.useMarker)(marker, {
      color: d.color
    })(0, 0, radius);
  };
...
function inferItemMarkerSize(scales, defaults) {
  ...
  if (scale instanceof scale_1.Identity) return scale.map(NaN) * 2;
  return defaults;
}
...
function inferCategoryStyle(options, context) {
  ...
  const finalLabelFormatter = typeof labelFormatter === 'string' ? (0, d3_format_1.format)(labelFormatter) : labelFormatter;
  const colorScale = (0, utils_1.scaleOf)(scales, 'color');
...
function inferLegendShape(value, options, component) {
  ...
  const {
    width,
    height
  } = (0, utils_1.inferComponentShape)(value, options, component);
  return {
    width,
    height
  };
...
const LegendCategory = options => {
  const {
    ...
    style = __rest(options, ["labelFormatter", "layout", "order", "orientation", "position", "size", "title", "cols", "itemMarker", "render"]);
  const {
    gridRow
  } = style;
  return context => {
    ...
    const {
      width,
      height
    } = inferLegendShape(value, options, exports.LegendCategory);
