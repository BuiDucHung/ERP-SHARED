...
class Runtime extends composition_1.CompositionNode {
   ...
   */
  options(options) {
    if (arguments.length === 0) return (0, utils_2.optionsOf)(this);
    const {
      type
    } = options;
  ...
  destroy() {
    ...
    if (this._container[utils_2.REMOVE_FLAG]) (0, utils_2.removeContainer)(this._container);
  ...
  forceFit() {
    ...
    const {
      width,
      height
    } = (0, utils_2.sizeOf)(this.options(), this._container);
  ...
  getDataByXY(point, options = {}) {
    ...
    try {
      ...
      if ((0, helper_1.isHeatmap)(element)) return (0, helper_1.dataOf)(element, dataMap.get(key));
  ...
  _create() {
    ...
    const compositions = ['composition.view',
    // chat.view()
    ...
    this._compositions = Object.fromEntries(compositions.map(key => {
      ...
      Composition = __decorate([(0, define_1.defineProps)((0, define_1.nodeProps)(this._marks))], Composition);
  ...
  _computedOptions() {
    const options = this.options();
    const {
      key = exports.G2_CHART_KEY
    } = options;
    const {
      width,
      height,
      depth
    } = (0, utils_2.sizeOf)(options, this._container);
    this._width = width;
    this._height = height;
    this._key = key;
