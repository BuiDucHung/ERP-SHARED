...
function renderHTMLLabel(datum, index, data, attr) {
  ...
  var elStr = isFunction(labelRender) ? getCallbackValue(labelRender, [datum, index, data, getLabelVector(datum.value, attr)]) : datum.label || '';
...
function applyTextStyle(node, style) {
  if (['text', 'html'].includes(node.nodeName)) node.attr(omit(style, STYLE_OMIT_MAP[node.nodeName]));
}
...
function renderLabel(container, datum, data, style, attr) {
  ...
  var label = select(container).append(labelRender ? renderHTMLLabel(datum, index, data, attr) : formatter(datum, index, data, attr)).attr('className', CLASS_NAMES.labelItem.name).node();
