...
function getLocalBBox(shape) {
  var _a = shape.getLocalBounds(),
    min = _a.min,
    max = _a.max;
  var _b = tslib_1.__read([min, max], 2),
    _c = tslib_1.__read(_b[0], 2),
    x1 = _c[0],
    y1 = _c[1],
    _d = tslib_1.__read(_b[1], 2),
    x2 = _d[0],
    y2 = _d[1];
  return {
    x: x1,
...
function createTempText(group, attrs) {
  ...
  textNode.attr(tslib_1.__assign(tslib_1.__assign({}, attrs), {
    visibility: 'hidden'
  }));
  return textNode;
}
function isHorizontal(p1, p2) {
  var _a = tslib_1.__read(p1, 2),
    x1 = _a[0],
    y1 = _a[1];
  var _b = tslib_1.__read(p2, 2),
    x2 = _b[0],
    y2 = _b[1];
  return x1 !== x2 && y1 === y2;
}
