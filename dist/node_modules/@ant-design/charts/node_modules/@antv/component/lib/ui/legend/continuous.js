...
function getMinMax(data) {
  return {
    min: Math.min.apply(Math, tslib_1.__spreadArray([], tslib_1.__read(data.map(function (d) {
      return d.value;
    })), false)),
    max: Math.max.apply(Math, tslib_1.__spreadArray([], tslib_1.__read(data.map(function (d) {
      return d.value;
    })), false))
  };
}
var Continuous = /** @class */function (_super) {
  ...
  function Continuous(options) {
    ...
    _this.onDragging = function (e) {
      ...
      _this.updateMouse();
      var _a = tslib_1.__read(_this.selection, 2),
        start = _a[0],
        end = _a[1];
  ...
  Continuous.prototype.render = function (attributes, container) {
    ...
    /** label */
    /** content */
    var contentGroup = (0, util_2.select)(container).maybeAppendByClassName(constant_2.CLASS_NAMES.contentGroup, 'g').styles({
      transform: "translate(".concat(x, ", ").concat(y, ")")
    });
    var labelGroup = contentGroup.maybeAppendByClassName(constant_2.CLASS_NAMES.labelGroup, 'g').styles({
      zIndex: 1
    });
    (0, util_2.ifShow)(!!showLabel, labelGroup, function (group) {
      _this.renderLabel(group);
    });
    var ribbonGroup = contentGroup.maybeAppendByClassName(constant_2.CLASS_NAMES.ribbonGroup, 'g').styles({
      zIndex: 0
    });
    /** handle */
    this.handlesGroup = contentGroup.maybeAppendByClassName(constant_2.CLASS_NAMES.handlesGroup, 'g').styles({
      zIndex: 2
    });
    this.renderHandles();
    /** ribbon */
  ...
  Object.defineProperty(Continuous.prototype, "ribbonRange", {
    get: function () {
      var _a = tslib_1.__read(this.selection, 2),
        min = _a[0],
        max = _a[1];
  ...
  Continuous.prototype.renderTitle = function (container) {
    ...
    var finalTitleStyle = tslib_1.__assign(tslib_1.__assign({}, style), {
      width: width,
      height: height,
      text: titleText,
    ...
    var that = this;
    container.selectAll(constant_2.CLASS_NAMES.title.class).data(showTitle ? [titleText] : []).join(function (enter) {
      return enter.append(function () {
        return new title_1.Title({
          style: finalTitleStyle
        });
      }).attr('className', constant_2.CLASS_NAMES.title.name).each(function () {
        that.title = this;
      });
    }, function (update) {
  ...
  Object.defineProperty(Continuous.prototype, "labelBBox", {
    get: function () {
      ...
      if (!showLabel) return new util_2.BBox(0, 0, 0, 0);
      if (this.cacheLabelBBox) return this.cacheLabelBBox;
      var _b = ((_a = this.label.querySelector(constant_1.CLASS_NAMES.labelGroup.class)) === null || _a === void 0 ? void 0 : _a.children.slice(-1)[0]).getBBox(),
        width = _b.width,
        height = _b.height;
  ...
  Object.defineProperty(Continuous.prototype, "ribbonBBox", {
    get: function () {
      ...
      var _d = tslib_1.__read(this.ifHorizontal([availableHeight, availableWidth], [availableWidth, availableHeight]), 2),
        availableSize = _d[0],
      ...
      var _f = tslib_1.__read(this.ifHorizontal([ribbonLength, ribbonSize], [ribbonSize, ribbonLength]), 2),
        width = _f[0],
        height = _f[1];
      // 需要考虑 handle 的占用空间
      // todo 为了防止因为 handle 文本变化导致的 ribbon 位置变化，handle size 取最大值
      var finalLabelOccupy = ['top', 'left'].includes(labelPosition) ? labelSize : 0;
      var _g = tslib_1.__read(this.ifHorizontal([edgeLength / 2, finalLabelOccupy], [finalLabelOccupy, edgeLength / 2]), 2),
        x = _g[0],
        y = _g[1];
  ...
  Continuous.prototype.renderRibbon = function (container) {
    ...
    this.ribbon = container.maybeAppendByClassName(constant_2.CLASS_NAMES.ribbon, function () {
  ...
  Continuous.prototype.renderHandles = function () {
    ...
    var _b = tslib_1.__read(this.selection, 2),
      min = _b[0],
      max = _b[1];
    var style = tslib_1.__assign(tslib_1.__assign({}, handleStyle), {
      orientation: orientation,
      classNamePrefix: classNamePrefix
    });
    ...
    this.handlesGroup.selectAll(constant_2.CLASS_NAMES.handle.class).data(showHandle ? [{
      value: min,
      type: 'start'
    }, {
      value: max,
  ...
  Continuous.prototype.adjustHandles = function () {
    ...
    var _b = this.attributes,
      classNamePrefix = _b.classNamePrefix,
      showHandle = _b.showHandle;
    var _c = (0, util_2.subStyleProps)(this.attributes, 'handle').shape,
      shape = _c === void 0 ? 'slider' : _c;
  ...
  Object.defineProperty(Continuous.prototype, "handleBBox", {
    get: function () {
      ...
      if (!this.attributes.showHandle) return new util_2.BBox(0, 0, 0, 0);
      ...
      var _c = tslib_1.__read([Math.max(startHandleWidth, endHandleWidth), Math.max(startHandleHeight, endHandleHeight)], 2),
        width = _c[0],
        height = _c[1];
  ...
  Object.defineProperty(Continuous.prototype, "handleShape", {
    ...
    get: function () {
      var _a = this.handleBBox,
        width = _a.width,
        height = _a.height;
      var _b = tslib_1.__read(this.ifHorizontal([height, width], [width, height]), 2),
        size = _b[0],
        length = _b[1];
      return {
        width: width,
  ...
  Continuous.prototype.setHandlePosition = function (type, value) {
    ...
    var _b = tslib_1.__read(this.ifHorizontal([ribbonX + offset, ribbonY + ribbonSize * this.handleOffsetRatio], [ribbonX + ribbonSize * this.handleOffsetRatio, ribbonY + offset]), 2),
      x = _b[0],
      y = _b[1];
  ...
  Continuous.prototype.renderIndicator = function (container) {
    ...
    this.indicator = container.maybeAppendByClassName(constant_2.CLASS_NAMES.indicator, function () {
  ...
  Object.defineProperty(Continuous.prototype, "labelData", {
    get: function () {
      ...
      return data.reduce(function (acc, curr, index, arr) {
        ...
        acc.push(tslib_1.__assign(tslib_1.__assign({}, curr), {
          id: id,
          index: index,
          type: 'value',
        ...
        if (index < arr.length - 1) {
          ...
          var _c = tslib_1.__read([curr.value, next.value], 2),
            cr = _c[0],
            nx = _c[1];
          var midVal = (cr + nx) / 2;
          acc.push(tslib_1.__assign(tslib_1.__assign({}, curr), {
  ...
  Object.defineProperty(Continuous.prototype, "labelStyle", {
    get: function () {
      var _a = tslib_1.__read(['center', 'middle'], 2),
        labelTextAlign = _a[0],
  ...
  Continuous.prototype.renderLabel = function (container) {
    ...
    var style = (0, util_2.deepAssign)(tslib_1.__assign({
      showLine: false,
      showGrid: false,
      showTick: showTick,
      type: 'linear',
    ...
    }, this.labelStyle), (0, util_2.superStyleProps)(tickStyle, 'tick'), (0, util_2.superStyleProps)(labelStyle, 'label'), {
      data: this.labelData
    });
    var functionStyle = {
    ...
    var finalLabelStyle = tslib_1.__assign(tslib_1.__assign(tslib_1.__assign({}, style), functionStyle), {
      labelOverlap: [{
        type: 'hide'
      }]
    });
    this.label = container.maybeAppendByClassName(constant_2.CLASS_NAMES.label, function () {
      return new axis_1.Axis({
  ...
  Continuous.prototype.adjustLabel = function () {
    ...
    var _c = tslib_1.__read(this.ifHorizontal([[x, y + axisOffset], [x + width, y + axisOffset]], [[x + axisOffset, y + height], [x + axisOffset, y]]), 2),
      startPos = _c[0],
      endPos = _c[1];
    this.label.update({
      startPos: startPos,
  ...
  Continuous.prototype.updateSelection = function (stVal, endVal, isOffset) {
    ...
    var _b = tslib_1.__read(this.selection, 2),
      currSt = _b[0],
      currEnd = _b[1];
    var _c = tslib_1.__read([stVal, endVal], 2),
      start = _c[0],
      end = _c[1];
    if (isOffset) {
      // 获取当前值
  ...
  Object.defineProperty(Continuous.prototype, "step", {
    get: function () {
      ...
      var _b = this.range,
        min = _b.min,
        max = _b.max;
      if ((0, util_1.isUndefined)(step)) return (0, util_2.toPrecision)((max - min) * constant_2.STEP_RATIO, 0);
      return step;
    },
    enumerable: false,
    configurable: true
  ...
  Continuous.prototype.getTickValue = function (value) {
    ...
    if (block) return (0, utils_1.getNextTickValue)(data.map(function (_a) {
      var value = _a.value;
      return value;
    }), value).tick;
  ...
  Continuous.prototype.getValueByCanvasPoint = function (e) {
    var _a = this.range,
      min = _a.min,
      max = _a.max;
    var _b = tslib_1.__read(this.ribbon.node().getPosition(), 2),
      x = _b[0],
      y = _b[1];
