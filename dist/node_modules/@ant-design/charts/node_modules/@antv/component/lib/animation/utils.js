...
function transitionShape(source, target, options, after) {
  ...
  var afterTransition = function () {
    if (after === 'destroy') source.destroy();else if (after === 'hide') (0, util_2.hide)(source);
    if (target.isVisible()) (0, util_2.show)(target);
  };
  if (!options) {
    afterTransition();
    return [null];
  ...
  var _c = tslib_1.__read(source.getGeometryBounds().center, 2),
    sx = _c[0],
    sy = _c[1];
  var _d = tslib_1.__read(target.getGeometryBounds().center, 2),
    ex = _d[0],
    ey = _d[1];
  var _e = tslib_1.__read([(sx + ex) / 2 - sx, (sy + ey) / 2 - sy], 2),
    mx = _e[0],
    my = _e[1];
...
function transition(target, state, options) {
  ...
  return animate(target, [from, to], tslib_1.__assign({
    fill: 'both'
  }, options));
}