...
function createCrossBBox(attr, padding) {
  ...
  var _c = tslib_1.__read(attr.startPos, 2),
    sx = _c[0],
    sy = _c[1],
    _d = tslib_1.__read(attr.endPos, 2),
    ex = _d[0],
    ey = _d[1];
  // 水平时取左右，垂直时取上下
  var _e = tslib_1.__read((0, line_1.isAxisVertical)(attr) ? [-padding, 0, padding, 0] : [0, padding, 0, -padding], 4),
    top = _e[0],
    right = _e[1],
    bottom = _e[2],
    left = _e[3];
...
function boundTest(items, attr, margin) {
  ...
  var testContain = function (item) {
    if (crossBBox) return (0, contain_1.contain)(crossBBox, item);
    return true;
  };
