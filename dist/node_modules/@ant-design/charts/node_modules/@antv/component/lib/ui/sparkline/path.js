...
function dataToLines(data, scales) {
  ...
  var x = scales.x,
    y = scales.y;
  var _b = tslib_1.__read(y.getOptions().range || [0, 0], 2),
    max = _b[0],
    min = _b[1];
  if (min > max) _a = tslib_1.__read([max, min], 2), min = _a[0], max = _a[1];
...
function lineToLinePath(line, reverse) {
  ...
  var linePath = line.map(function (point, idx) {
    return tslib_1.__spreadArray([idx === M ? 'M' : 'L'], tslib_1.__read(point), false);
  });
