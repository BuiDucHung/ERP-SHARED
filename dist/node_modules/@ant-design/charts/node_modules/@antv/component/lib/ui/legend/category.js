...
var Category = /** @class */function (_super) {
  ...
  Category.prototype.renderTitle = function (container, width, height) {
    ...
    var _b = tslib_1.__read((0, util_1.splitStyle)(style), 2),
      titleStyle = _b[0],
      groupStyle = _b[1];
    this.titleGroup = container.maybeAppendByClassName(constant_1.CLASS_NAMES.titleGroup, 'g').styles(groupStyle);
    var finalTitleStyle = tslib_1.__assign(tslib_1.__assign({
      width: width,
      height: height
    }, titleStyle), {
    ...
    this.title = this.titleGroup.maybeAppendByClassName(constant_1.CLASS_NAMES.title, function () {
  ...
  Category.prototype.renderCustom = function (container) {
    ...
    container.maybeAppendByClassName(constant_1.CLASS_NAMES.html, function () {
  ...
  Category.prototype.renderItems = function (container, bbox) {
    ...
    var itemStyle = tslib_1.__assign(tslib_1.__assign({}, partialItemStyle), {
      width: width,
      height: height,
      x: 0,
      y: 0
    });
    this.itemsGroup = container.maybeAppendByClassName(constant_1.CLASS_NAMES.itemsGroup, 'g').styles(tslib_1.__assign(tslib_1.__assign({}, groupStyle), {
      transform: "translate(".concat(x, ", ").concat(y, ")")
    }));
    var that = this;
    this.itemsGroup.selectAll(constant_1.CLASS_NAMES.items.class).data(['items']).join(function (enter) {
      return enter.append(function () {
        return new items_1.CategoryItems({
          style: itemStyle
        });
      }).attr('className', constant_1.CLASS_NAMES.items.name).each(function () {
  ...
  Object.defineProperty(Category.prototype, "availableSpace", {
    get: function () {
      var _a = this.attributes,
        ...
        width = _a.width,
        height = _a.height;
      if (!showTitle) return new util_1.BBox(0, 0, width, height);
