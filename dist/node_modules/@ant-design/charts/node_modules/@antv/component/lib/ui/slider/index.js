...
var Slider = /** @class */function (_super) {
  ...
  function Slider(options) {
    var _this = _super.call(this, options, tslib_1.__assign(tslib_1.__assign(tslib_1.__assign({
      x: 0,
      y: 0,
      animate: {
        duration: 100,
      ...
      handleIconOffset: 0
    }, (0, util_2.superStyleProps)(constant_1.HANDLE_DEFAULT_CFG, 'handle')), (0, util_2.superStyleProps)(constant_1.HANDLE_ICON_DEFAULT_CFG, 'handleIcon')), (0, util_2.superStyleProps)(constant_1.HANDLE_LABEL_DEFAULT_CFG, 'handleLabel'))) || this;
    _this.range = [0, 1];
    _this.onDragStart = function (target) {
      return function (e) {
        e.stopPropagation();
        _this.target = target;
        _this.prevPos = _this.getOrientVal((0, util_2.getEventViewportPos)(e));
    ...
    _this.onDragging = function (e) {
      var _a = _this.attributes,
        ...
        type = _a.type;
      e.stopPropagation();
      var currPos = _this.getOrientVal((0, util_2.getEventViewportPos)(e));
  ...
  Object.defineProperty(Slider.prototype, "shape", {
    get: function () {
      ...
      var _b = tslib_1.__read(this.getOrientVal([[trackLength, trackSize], [trackSize, trackLength]]), 2),
        width = _b[0],
        height = _b[1];
      return {
        width: width,
  ...
  Object.defineProperty(Slider.prototype, "availableSpace", {
    get: function () {
      var _a = this.attributes,
        ...
        y = _a.y,
        padding = _a.padding;
      var _b = tslib_1.__read((0, util_2.parseSeriesAttr)(padding), 4),
        top = _b[0],
        right = _b[1],
        bottom = _b[2],
        left = _b[3];
  ...
  Slider.prototype.renderTrack = function (container) {
    ...
    this.trackShape = (0, util_2.select)(container).maybeAppendByClassName(constant_1.CLASS_NAMES.track, 'rect').styles(tslib_1.__assign(tslib_1.__assign({
      x: x,
      y: y
    }, this.shape), style));
  };
  Slider.prototype.renderBrushArea = function (container) {
    var _a = this.attributes,
      ...
      y = _a.y,
      brushable = _a.brushable;
    this.brushArea = (0, util_2.select)(container).maybeAppendByClassName(constant_1.CLASS_NAMES.brushArea, 'rect').styles(tslib_1.__assign({
      x: x,
      y: y,
      fill: 'transparent',
  ...
  Slider.prototype.renderSparkline = function (container) {
    ...
    (0, util_2.ifShow)(orientation === 'horizontal', sparklineGroup, function (group) {
      var style = tslib_1.__assign(tslib_1.__assign({}, _this.sparklineStyle), {
        x: x,
        y: y
      });
      group.maybeAppendByClassName(constant_1.CLASS_NAMES.sparkline, function () {
        return new sparkline_1.Sparkline({
          style: style
        });
      }).update(style);
    });
  ...
  Slider.prototype.renderHandles = function () {
    ...
    var that = this;
    (_a = this.foregroundGroup) === null || _a === void 0 ? void 0 : _a.selectAll(constant_1.CLASS_NAMES.handle.class).data(data.map(function (type) {
      return {
        type: type
      };
    }), function (d) {
  ...
  Slider.prototype.renderSelection = function (container) {
    ...
    var applyStyle = function (selection) {
      return selection.style('visibility', function (d) {
        ...
        return 'default';
      }).styles(tslib_1.__assign(tslib_1.__assign({}, selectionStyle), {
        transform: "translate(".concat(x, ", ").concat(y, ")")
      }));
    };
    var that = this;
    this.foregroundGroup.selectAll(constant_1.CLASS_NAMES.selection.class).data(type === 'value' ? [] : this.calcSelectionArea().map(function (area, index) {
      return {
      ...
      return enter.append('rect').attr('className', constant_1.CLASS_NAMES.selection.name).call(applyStyle).each(function (datum, index) {
        var _this = this;
        if (index === 1) {
  ...
  Slider.prototype.clampValues = function (values, precision) {
    ...
    var _b = tslib_1.__read(this.range, 2),
      min = _b[0],
      max = _b[1];
    var _c = tslib_1.__read(this.getValues().map(function (num) {
        return (0, util_2.toPrecision)(num, precision);
      }), 2),
      prevStart = _c[0],
      prevEnd = _c[1];
    var internalValues = Array.isArray(values) ? values : [prevStart, values !== null && values !== void 0 ? values : prevEnd];
    var _d = tslib_1.__read((internalValues || [prevStart, prevEnd]).map(function (num) {
        return (0, util_2.toPrecision)(num, precision);
      }), 2),
      startVal = _d[0],
      endVal = _d[1];
    if (this.attributes.type === 'value') return [0, (0, util_1.clamp)(endVal, min, max)];
    // 交换startVal endVal
    if (startVal > endVal) {
  ...
  Slider.prototype.calcSelectionArea = function (values) {
    var _a = tslib_1.__read(this.clampValues(values), 2),
      start = _a[0],
      end = _a[1];
  ...
  Slider.prototype.calcHandlePosition = function (handleType) {
    ...
    var _a = this.availableSpace,
      ...
      width = _a.width,
      height = _a.height;
    var _b = tslib_1.__read(this.clampValues(), 2),
      stVal = _b[0],
      endVal = _b[1];
  ...
  Slider.prototype.calcHandleText = function (handleType) {
    ...
    var temp = new shapes_1.Text({
      style: tslib_1.__assign(tslib_1.__assign(tslib_1.__assign({}, labelStyle), this.inferTextStyle(handleType)), {
        text: text
      })
    });
    // 文字包围盒的宽高
  ...
  Slider.prototype.getHandleStyle = function (handleType) {
    ...
    return tslib_1.__assign(tslib_1.__assign(tslib_1.__assign({}, (0, util_2.superStyleProps)(this.getHandleIconStyle(), 'icon')), (0, util_2.superStyleProps)(tslib_1.__assign(tslib_1.__assign({}, this.getHandleLabelStyle(handleType)), textStyle), 'label')), {
  ...
  Slider.prototype.setValuesOffset = function (stOffset, endOffset) {
    ...
    var _a = tslib_1.__read(this.getValues(), 2),
      oldStartVal = _a[0],
      oldEndVal = _a[1];
  ...
  Slider.prototype.dispatchCustomEvent = function (target, event, name) {
    ...
    target.on(event, function (e) {
      e.stopPropagation();
      _this.dispatchEvent(new g_1.CustomEvent(name, {
        detail: e
      }));
    });
  };
