...
var Select = /** @class */function (_super) {
  ...
  Select.prototype.renderSelect = function () {
    ...
    this.select.attr(tslib_1.__assign(tslib_1.__assign({
      x: x,
      y: y,
      width: width,
      height: height
    ...
    var iconSize = 10;
    if (showDropdownIcon) {
      (0, util_1.select)(this.select).maybeAppend('.dropdown-icon', 'path').style('d', 'M-5,-3.5 L0,3.5 L5,-3.5').style('transform', "translate(".concat(x + width - iconSize - padding[1] - padding[3], ", ").concat(y + height / 2, ")")).style('lineWidth', 1).style('stroke', this.select.style.stroke);
    }
    ...
    // placeholder
    var finalPlaceholderStyle = tslib_1.__assign({
      x: x + padding[3]
    }, placeholderStyle);
    (0, util_1.select)(this.select).selectAll('.placeholder').data(!currentOption ? [1] : []).join(function (enter) {
    ...
    var finalValueStyle = tslib_1.__assign({
      x: x + padding[3]
    }, labelStyle);
    (0, util_1.select)(this.select).selectAll('.value').data(currentOption ? [currentOption] : []).join(function (enter) {
      return enter.append(function (datum) {
        return (0, util_1.renderExtDo)(datum.label);
  ...
  Select.prototype.renderDropdown = function () {
    ...
    var padding = this.dropdownPadding;
    (0, util_1.select)(this.dropdown).maybeAppend('.dropdown-container', 'g').attr('className', 'dropdown-container').selectAll('.dropdown-item').data(options, function (datum) {
      return datum.value;
    }).join(function (enter) {
      return enter.append(function (datum) {
        return new option_1.Option({
          className: 'dropdown-item',
          style: tslib_1.__assign(tslib_1.__assign(tslib_1.__assign({}, datum), optionStyle), {
            ...
            onClick: function (value, option, item) {
              ...
              _this.dispatchEvent(new g_1.CustomEvent('change', {
                detail: {
                  value: value,
                  option: option,
                  item: item
    ...
    this.dropdown.attr(tslib_1.__assign({
  ...
  Select.prototype.bindEvents = function () {
    ...
    this.select.addEventListener('click', function () {
      if (_this.dropdown.style.visibility === 'visible') (0, util_1.hide)(_this.dropdown);else {
