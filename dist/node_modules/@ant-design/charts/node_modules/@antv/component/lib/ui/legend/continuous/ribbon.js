...
function getTrackPath(attr) {
  var type = attr.type;
  var _a = tslib_1.__read(getShape(attr), 2),
    cw = _a[0],
    ch = _a[1];
  if (type === 'size') {
...
function getColor(attr) {
  ...
  var genericColor = colors.map(function (c) {
    return (0, g_1.parseColor)(c).toString();
  });
  if (!count) return '';
  if (count === 1) return genericColor[0];
  if (block) return (0, utils_2.getBlockColor)(partition, genericColor, orientation);
  return genericColor.reduce(function (r, c, idx) {
    return r += " ".concat(idx / (count - 1), ":").concat(c);
  }, "l(".concat((0, utils_1.ifHorizontal)(orientation, '0', '270'), ")"));
}
...
function renderTrack(container, attr) {
  ...
  var trackClassName = (0, classname_1.getLegendClassName)(CLASS_NAMES.track.name, constant_1.CLASSNAME_SUFFIX_MAP.track, classNamePrefix);
  container.maybeAppendByClassName(CLASS_NAMES.track, 'path').attr('className', trackClassName).styles(tslib_1.__assign({
    d: getTrackPath(attr)
  }, style));
}
function renderSelection(container, attr) {
  ...
  var classNamePrefix = attr.classNamePrefix;
  var selectionClassName = (0, classname_1.getLegendClassName)(CLASS_NAMES.selection.name, constant_1.CLASSNAME_SUFFIX_MAP.selection, classNamePrefix);
  var ribbon = container.maybeAppendByClassName(CLASS_NAMES.selection, 'path').attr('className', selectionClassName).styles(tslib_1.__assign({
