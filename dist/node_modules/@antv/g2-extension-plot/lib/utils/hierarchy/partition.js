...
function getAllNodes(root) {
  ...
  if (root && root.each) {
    ...
    root.each(node => {
      ...
      const ancestors = (0, util_1.filter)((((_a = node.ancestors) === null || _a === void 0 ? void 0 : _a.call(node)) || []).map(d => nodes.find(n => n.name === d.name) || d), ({
        depth
...
function partition(data, options) {
   ...
   */
  d3Hierarchy.hierarchy(data).sum(d => (0, util_1.size)(d.children) ? options.ignoreParentValue ? 0 : d[field] - (0, util_1.reduce)(d.children, (a, b) => a + b[field], 0) : d[field]).sort(options.sort));
