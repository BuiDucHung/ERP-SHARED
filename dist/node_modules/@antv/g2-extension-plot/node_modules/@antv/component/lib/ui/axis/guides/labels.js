...
function getAngle(v1, v2) {
  var _a = tslib_1.__read(v1, 2),
    x1 = _a[0],
    y1 = _a[1];
  var _b = tslib_1.__read(v2, 2),
    x2 = _b[0],
    y2 = _b[1];
  var _c = tslib_1.__read([x1 * x2 + y1 * y2, x1 * y2 - y1 * x2], 2),
    dot = _c[0],
    det = _c[1];
...
function correctLabelRotation(_rotate) {
  ...
  if (!(0, util_2.inRange)(rotate, -90, 90)) rotate += 180;
  return rotate;
}
...
function getLabelPos(datum, data, attr) {
  ...
  var _a = tslib_1.__read([(0, utils_2.getLabelVector)(datum.value, attr), (0, utils_1.getFactor)(labelDirection, tickDirection)], 2),
    labelVector = _a[0],
    unionFactor = _a[1];
  var extraLength = unionFactor === 1 ? (0, util_2.getCallbackValue)(showTick ? tickLength : 0, [datum, index, data]) : 0;
  var _b = tslib_1.__read((0, util_2.add)((0, util_2.scale)(labelVector, finalLabelSpacing + extraLength), (0, line_1.getValuePos)(datum.value, attr)), 2),
    x = _b[0],
    y = _b[1];
  return {
    x: x,
...
function formatter(datum, index, data, attr) {
  ...
  var element = (0, util_1.isFunction)(labelFormatter) ? function () {
    return (0, util_2.renderExtDo)((0, util_2.getCallbackValue)(labelFormatter, [datum, index, data, (0, utils_2.getLabelVector)(datum.value, attr)]));
  } : function () {
    return (0, util_2.renderExtDo)(datum.label || '');
  };
  return element;
}
function renderHTMLLabel(datum, index, data, attr) {
  ...
  var elStr = (0, util_1.isFunction)(labelRender) ? (0, util_2.getCallbackValue)(labelRender, [datum, index, data, (0, utils_2.getLabelVector)(datum.value, attr)]) : datum.label || '';
...
function applyTextStyle(node, style) {
  if (['text', 'html'].includes(node.nodeName)) node.attr((0, util_2.omit)(style, STYLE_OMIT_MAP[node.nodeName]));
}
...
function renderLabel(container, datum, data, style, attr) {
  ...
  var label = (0, util_2.select)(container).append(labelRender ? renderHTMLLabel(datum, index, data, attr) : formatter(datum, index, data, attr)).attr('className', constant_1.CLASS_NAMES.labelItem.name).node();
  ...
  var _a = tslib_1.__read((0, util_2.splitStyle)((0, utils_2.getCallbackStyle)(style, [datum, index, data])), 2),
    labelStyle = _a[0],
    _b = _a[1],
    transform = _b.transform,
    groupStyle = tslib_1.__rest(_b, ["transform"]);
...
function renderLabels(container, data, attr, animate, main) {
  ...
  var _exit;
  var transitions = container.selectAll(constant_1.CLASS_NAMES.label.class).data(finalData, function (d, i) {
    return i;
  }).join(function (enter) {
    var labels = enter.append('g').attr('className', constant_1.CLASS_NAMES.label.name).transition(function (datum) {
    ...
    return update.transition(function (datum) {
      ...
      var _a = getLabelPos(datum, data, attr),
        x = _a.x,
        y = _a.y;
      var animation = (0, animation_1.transition)(this, {
    ...
    exit.transition(function () {
      ...
      (0, animation_1.onAnimateFinished)(animation, function () {
        return (0, util_2.select)(_this).remove();
      });
      return animation;
    });
    return _exit;
