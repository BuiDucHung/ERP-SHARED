...
function getBBox(title, content) {
  ...
  var _b = tslib_1.__read((0, util_1.parseSeriesAttr)(text ? spacing : 0), 4),
    spacingTop = _b[0],
    spacingRight = _b[1],
    spacingBottom = _b[2],
    spacingLeft = _b[3];
  var _c = tslib_1.__read((0, util_1.parseSeriesAttr)(inset), 4),
    insetTop = _c[0],
    insetRight = _c[1],
    insetBottom = _c[2],
    insetLeft = _c[3];
  var _d = tslib_1.__read([spacingLeft + spacingRight, spacingTop + spacingBottom], 2),
    spacingWidth = _d[0],
    spacingHeight = _d[1];
  var _e = tslib_1.__read([insetLeft + insetRight, insetTop + insetBottom], 2),
    insetWidth = _e[0],
    insetHeight = _e[1];
  // 只基于第一个 pos 进行判断
  ...
  var _f = tslib_1.__read([content.attributes.width || contentBBox.width, content.attributes.height || contentBBox.height], 2),
    contentWidth = _f[0],
    contentHeight = _f[1];
...
function getTitleLayout(cfg) {
  ...
  var _f = tslib_1.__read([+width / 2, +height / 2], 2),
    hW = _f[0],
    hH = _f[1];
  var _g = tslib_1.__read([+hW, +hH, 'center', 'middle'], 4),
    x = _g[0],
    y = _g[1],
    textAlign = _g[2],
    textBaseline = _g[3];
  var pos = (0, util_1.parsePosition)(position);
  if (pos.includes('l')) _a = tslib_1.__read([0, 'start'], 2), x = _a[0], textAlign = _a[1];
  if (pos.includes('r')) _b = tslib_1.__read([+width, 'end'], 2), x = _b[0], textAlign = _b[1];
  if (pos.includes('t')) _c = tslib_1.__read([0, 'top'], 2), y = _c[0], textBaseline = _c[1];
  if (pos.includes('b')) _d = tslib_1.__read([+height, 'bottom'], 2), y = _d[0], textBaseline = _d[1];
  return {
    x: x,
    y: y,
    textAlign: textAlign,
...
var Title = /** @class */function (_super) {
  ...
  Title.prototype.getAvailableSpace = function () {
    ...
    if (!title) return new util_1.BBox(0, 0, +containerWidth, +containerHeight);
    var _b = title.getBBox(),
      titleWidth = _b.width,
      titleHeight = _b.height;
    var _c = tslib_1.__read((0, util_1.parseSeriesAttr)(spacing), 4),
      spacingTop = _c[0],
      spacingRight = _c[1],
      spacingBottom = _c[2],
      spacingLeft = _c[3];
    var _d = tslib_1.__read([0, 0, +containerWidth, +containerHeight], 4),
      x = _d[0],
      y = _d[1],
      width = _d[2],
      height = _d[3];
    var pos = (0, util_1.parsePosition)(position);
    if (pos.includes('i')) return new util_1.BBox(x, y, width, height);
    pos.forEach(function (p, i) {
      var _a, _b, _c, _d;
      if (p === 't') _a = tslib_1.__read(i === 0 ? [titleHeight + spacingBottom, +containerHeight - titleHeight - spacingBottom] : [0, +containerHeight], 2), y = _a[0], height = _a[1];
      if (p === 'r') _b = tslib_1.__read([+containerWidth - titleWidth - spacingLeft], 1), width = _b[0];
      if (p === 'b') _c = tslib_1.__read([+containerHeight - titleHeight - spacingTop], 1), height = _c[0];
      if (p === 'l') _d = tslib_1.__read(i === 0 ? [titleWidth + spacingRight, +containerWidth - titleWidth - spacingRight] : [0, +containerWidth], 2), x = _d[0], width = _d[1];
    });
    var _e = tslib_1.__read((0, util_1.parseSeriesAttr)(inset), 4),
      insetTop = _e[0],
      insetRight = _e[1],
      insetBottom = _e[2],
      insetLeft = _e[3];
    var _f = tslib_1.__read([insetLeft + insetRight, insetTop + insetBottom], 2),
      insetWidth = _f[0],
      insetHeight = _f[1];
  ...
  Title.prototype.render = function (attributes, container) {
    ...
    var width = attributes.width,
      ...
      classNamePrefix = attributes.classNamePrefix,
      restStyle = tslib_1.__rest(attributes, ["width", "height", "position", "spacing", "classNamePrefix"]);
    var _a = tslib_1.__read((0, util_1.splitStyle)(restStyle), 1),
      titleStyle = _a[0];
