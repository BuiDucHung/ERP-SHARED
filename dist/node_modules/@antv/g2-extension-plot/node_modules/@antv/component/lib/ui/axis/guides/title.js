...
function getTitlePosition(mainGroup, titleGroup, attr) {
  ...
  var _b = mainGroup.node().getLocalBounds(),
    _c = tslib_1.__read(_b.min, 2),
    mainX = _c[0],
    mainY = _c[1],
    _d = tslib_1.__read(_b.halfExtents, 2),
    mainHalfWidth = _d[0],
    mainHalfHeight = _d[1];
  var _e = tslib_1.__read(titleGroup.node().getLocalBounds().halfExtents, 2),
    titleHalfWidth = _e[0],
    titleHalfHeight = _e[1];
  var _f = tslib_1.__read([mainX + mainHalfWidth, mainY + mainHalfHeight], 2),
    x = _f[0],
    y = _f[1];
  var _g = tslib_1.__read((0, util_1.parseSeriesAttr)(spacing), 4),
    spacingTop = _g[0],
    spacingRight = _g[1],
    spacingBottom = _g[2],
    spacingLeft = _g[3];
  if (['start', 'end'].includes(position) && attr.type === 'linear') {
    ...
    var _h = tslib_1.__read(position === 'start' ? [startPos, endPos] : [endPos, startPos], 2),
      from = _h[0],
      to = _h[1];
    var direction = (0, util_1.normalize)([-to[0] + from[0], -to[1] + from[1]]);
    var _j = tslib_1.__read((0, util_1.scale)(direction, spacingTop), 2),
      dx = _j[0],
      dy = _j[1];
    return {
      x: from[0] + dx,
...
function applyTitleStyle(title, group, axis, attr, animate) {
  ...
  var _a = tslib_1.__read((0, util_1.splitStyle)(style), 2),
    titleStyle = _a[0],
    _b = _a[1],
    specified = _b.transform,
    transformOrigin = _b.transformOrigin,
    groupStyle = tslib_1.__rest(_b, ["transform", "transformOrigin"]);
  group.styles(groupStyle);
  var transform = specified || inferTransform(title.node(), titleStyle.direction, titleStyle.position);
  title.styles(tslib_1.__assign(tslib_1.__assign({}, titleStyle), {
    transformOrigin: transformOrigin
  }));
  (0, util_1.percentTransform)(title.node(), transform);
  var _c = getTitlePosition(
    // @ts-ignore
    (0, util_1.select)(axis._offscreen || axis.querySelector(constant_1.CLASS_NAMES.mainGroup.class)), group, attr),
    x = _c.x,
    y = _c.y;
  var animation = (0, animation_1.transition)(group.node(), {
...
function renderTitle(container, axis, attr, animate) {
  ...
  return container.selectAll(constant_1.CLASS_NAMES.title.class).data([{
    title: titleText
  }].filter(function (d) {
    return !!d.title;
  }), function (d, i) {
    ...
    var titles = enter.append(function () {
      return (0, util_1.renderExtDo)(titleText);
    }).attr('className', constant_1.CLASS_NAMES.title.name).transition(function () {
