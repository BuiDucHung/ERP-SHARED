...
function getTitleShapeBBox(titleShape) {
  ...
  if ((titleShape === null || titleShape === void 0 ? void 0 : titleShape.tagName) === 'html') {
    ...
    var _b = titleShape.getLocalBounds(),
      min = _b.min,
      halfExtents = _b.halfExtents;
    box = {
      left: min[0],
      top: min[1],
...
function renderGroup(container, className, x, y) {
  return (0, util_1.maybeAppend)(container, ".".concat(className), 'g').styles({
    className: className,
    x: x,
    y: y
  }).node();
...
function renderRect(container, className, width, height, style) {
  ...
  return (0, util_1.select)(container).maybeAppendByClassName(className, 'rect').styles(tslib_1.__assign({
    zIndex: -1,
    width: width,
    height: height
  }, style)).node();
