...
function getLinearValuePos(value, attr) {
  var _a = tslib_1.__read(attr.startPos, 2),
    sx = _a[0],
    sy = _a[1],
    _b = tslib_1.__read(attr.endPos, 2),
    ex = _b[0],
    ey = _b[1];
  var _c = tslib_1.__read([ex - sx, ey - sy], 2),
    dx = _c[0],
    dy = _c[1];
...
function getArcValuePos(value, attr) {
  var radius = attr.radius,
    _a = tslib_1.__read(attr.center, 2),
    cx = _a[0],
    cy = _a[1];
...
function getArcPath(startAngle, endAngle, cx, cy, radius) {
  ...
  var _a = tslib_1.__read([radius, radius], 2),
    rx = _a[0],
    ry = _a[1];
  var _b = tslib_1.__read([(0, util_2.degToRad)(startAngle), (0, util_2.degToRad)(endAngle)], 2),
    startAngleRadians = _b[0],
    endAngleRadians = _b[1];
  var getPosByAngle = function (angle) {
    return [cx + radius * Math.cos(angle), cy + radius * Math.sin(angle)];
  };
  var _c = tslib_1.__read(getPosByAngle(startAngleRadians), 2),
    x1 = _c[0],
    y1 = _c[1];
  var _d = tslib_1.__read(getPosByAngle(endAngleRadians), 2),
    x2 = _d[0],
    y2 = _d[1];
  if (isCircle(startAngle, endAngle)) {
    var middleAngleRadians = (endAngleRadians + startAngleRadians) / 2;
    var _e = tslib_1.__read(getPosByAngle(middleAngleRadians), 2),
      xm = _e[0],
      ym = _e[1];
...
function renderArc(container, attr, style, animate) {
  var startAngle = attr.startAngle,
    ...
    center = attr.center,
    radius = attr.radius,
    classNamePrefix = attr.classNamePrefix;
  return container.selectAll(constant_1.CLASS_NAMES.line.class).data([{
    d: getArcPath.apply(void 0, tslib_1.__spreadArray(tslib_1.__spreadArray([startAngle, endAngle], tslib_1.__read(center), false), [radius], false))
  }], function (d, i) {
    return i;
  }).join(function (enter) {
    var line = enter.append('path').attr('className', constant_1.CLASS_NAMES.line.name).styles(attr).styles({
      d: function (d) {
        return d.d;
      }
    });
    (0, classname_1.applyClassName)(line, constant_1.CLASS_NAMES.line, classname_map_1.CLASSNAME_SUFFIX_MAP.line, classNamePrefix);
    return line;
  }, function (update) {
    return update.transition(function () {
      ...
      if (animation) {
        var layout = function () {
          ...
          _this.style.d = getArcPath.apply(void 0, tslib_1.__spreadArray([], tslib_1.__read(data), false));
        };
...
function extendLine(startPos, endPos, range) {
  ...
  var _a = tslib_1.__read([startPos, endPos, range], 3),
    _b = tslib_1.__read(_a[0], 2),
    x1 = _b[0],
    y1 = _b[1],
    _c = tslib_1.__read(_a[1], 2),
    x2 = _c[0],
    y2 = _c[1],
    _d = tslib_1.__read(_a[2], 2),
    l1 = _d[0],
    l2 = _d[1];
  var _e = tslib_1.__read([x2 - x1, y2 - y1], 2),
    x = _e[0],
    y = _e[1];
  var L = Math.sqrt(Math.pow(x, 2) + Math.pow(y, 2));
  var _f = tslib_1.__read([-l1 / L, l2 / L], 2),
    s1 = _f[0],
    s2 = _f[1];
  return [s1 * x, s1 * y, s2 * x, s2 * y];
}
function getLinePath(points) {
  var _a = tslib_1.__read(points, 2),
    _b = tslib_1.__read(_a[0], 2),
    x1 = _b[0],
    y1 = _b[1],
    _c = tslib_1.__read(_a[1], 2),
...
function renderLinear(container, attr, style, animate) {
  var showTrunc = attr.showTrunc,
    ...
    classNamePrefix = attr.classNamePrefix;
  var _a = tslib_1.__read([startPos, endPos], 2),
    _b = tslib_1.__read(_a[0], 2),
    x1 = _b[0],
    y1 = _b[1],
    _c = tslib_1.__read(_a[1], 2),
    x2 = _c[0],
    y2 = _c[1];
  var _d = tslib_1.__read(lineExtension ? extendLine(startPos, endPos, lineExtension) : new Array(4).fill(0), 4),
    ox1 = _d[0],
    oy1 = _d[1],
    ox2 = _d[2],
    oy2 = _d[3];
  var renderLine = function (data) {
    return container.selectAll(constant_1.CLASS_NAMES.line.class).data(data, function (d, i) {
      return i;
    }).join(function (enter) {
      ...
      lines.attr('className', function (d) {
        if (!classNamePrefix) {
          return "".concat(constant_1.CLASS_NAMES.line.name, " ").concat(d.className);
        }
        var baseLineClassName = (0, classname_1.getAxisClassName)(constant_1.CLASS_NAMES.line.name, classname_map_1.CLASSNAME_SUFFIX_MAP.line, classNamePrefix);
        if (d.className === constant_1.CLASS_NAMES.lineFirst.name) {
          var specificClassName = (0, classname_1.getAxisClassName)(constant_1.CLASS_NAMES.lineFirst.name, classname_map_1.CLASSNAME_SUFFIX_MAP.lineFirst, classNamePrefix);
          return "".concat(baseLineClassName, " ").concat(specificClassName);
        }
        if (d.className === constant_1.CLASS_NAMES.lineSecond.name) {
          var specificClassName = (0, classname_1.getAxisClassName)(constant_1.CLASS_NAMES.lineSecond.name, classname_map_1.CLASSNAME_SUFFIX_MAP.lineSecond, classNamePrefix);
          return "".concat(baseLineClassName, " ").concat(specificClassName);
        }
  ...
  var _e = tslib_1.__read(truncRange, 2),
    r1 = _e[0],
    r2 = _e[1];
  var dx = x2 - x1;
  var dy = y2 - y1;
  var _f = tslib_1.__read([x1 + dx * r1, y1 + dy * r1], 2),
    x3 = _f[0],
    y3 = _f[1];
  var _g = tslib_1.__read([x1 + dx * r2, y1 + dy * r2], 2),
    x4 = _g[0],
    y4 = _g[1];
...
function renderAxisLine(container, attr, animate) {
  ...
  if (type === 'linear') animation = renderLinear(container, attr, (0, util_2.omit)(style, 'arrow'), animate);else animation = renderArc(container, attr, (0, util_2.omit)(style, 'arrow'), animate);
