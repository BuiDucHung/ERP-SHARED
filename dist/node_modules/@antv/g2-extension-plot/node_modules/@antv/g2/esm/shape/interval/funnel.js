...
export const Funnel = (options, context) => {
  ...
  return (points, value, defaults, point2d) => {
    ...
    const path = standardDirRadius.find(r => r > 0) ? createRoundedPath([p0, p1, p2, p3], standardDirRadius) : line().curve(curveLinearClosed)([p0, p1, p2, p3]);
    return select(document.createElement('path', {})).call(applyStyle, rest).style('d', path).style('fill', color).style('fillOpacity', opacity).call(applyStyle, style).node();
  };
};
Funnel.props = {
