...
function plotView(view, selection, transitions, context) {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    .append('rect').style('zIndex', 0).style('fill', 'transparent').attr('className', PLOT_CLASS_NAME).call(updateBBox).call(updateLayers, Array.from(markState.keys())).call(applyClip, clip), update => update.call(updateLayers, Array.from(markState.keys())).call(updateBBox)
    // .call((selection) => {
...
function plotBreak(view, selection, library, context) {
  ...
  // Render all breaks.
  select(breakLayer).selectAll(className(BREAK_CLASS_NAME)).data(breaks, d => d.key).join(enter => enter.append((d, index) => breaksShapeFunction.get(d)(d, index)).attr('className', BREAK_CLASS_NAME), update => update.each(function (d, i, element) {
