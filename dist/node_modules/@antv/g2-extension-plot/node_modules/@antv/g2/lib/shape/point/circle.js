...
const BaseCircle = (options, context) => {
  ...
  return (points, value, defaults) => {
    ...
    return (0, selection_1.select)(document.createElement('circle', {})).call(utils_1.applyStyle, defaults).style('fill', 'transparent').style('cx', cx).style('cy', cy).style('r', finalRadius).style('lineWidth', finalLineWidth).style('transform', transform).style('transformOrigin', `${cx} ${cy}`).style('stroke', color).style((0, utils_1.toOpacityKey)(options), opacity).style(colorAttribute, color).call(utils_1.applyStyle, style).node();
  };
};
...
const Circle = (options, context) => {
  return (0, exports.BaseCircle)(Object.assign({
    colorAttribute: 'fill'
  }, options), context);
};
