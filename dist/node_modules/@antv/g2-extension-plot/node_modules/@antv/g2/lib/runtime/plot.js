  ...
  return __awaiter(this, void 0, void 0, function* () {
    ...
    while (discovered.length) {
      ...
      if (isStandardView(node)) {
        ...
        const transformedNodes = children.flatMap(transform).map(d => (0, coordinate_1.coordinate2Transform)(d, library));
    ...
    const transitions = [];
    selection.selectAll(className(constant_1.VIEW_CLASS_NAME)).data(views, d => d.key).join(enter => enter.append('g').attr('className', constant_1.VIEW_CLASS_NAME).attr('id', view => view.key).call(applyTranslate).each(function (view, i, element) {
    ...
    const finished = transitions.filter(helper_1.defined).map(cancel).map(d => d.finished);
...
function transformMarks(options, context) {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    const {
      width,
      height
    } = (0, layout_1.computeRoughPlotSize)(options);
    const markOptions = {
      options,
      width,
      height
    ...
    while (discovered.length) {
      ...
      const {
        type = (0, helper_1.error)('G2Mark type is required.'),
        key
      } = mark;
      // For components.
...
function initializeMarks(options, context) {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    const scaleChannels = (0, d3_array_1.group)(Array.from(markState.values()).flatMap(d => d.channels), ({
      scaleKey
    }) => scaleKey);
    ...
    for (const channels of scaleChannels.values()) {
      ...
      const scale = Object.assign(Object.assign({}, (0, scale_1.inferScale)(name, values, options, coordinates, theme, library)), {
        uid: Symbol('scale'),
        key: scaleKey
      });
...
function useThemeInteraction(view, type, option, useInteraction) {
  ...
  const interaction = useInteraction((0, util_1.deepMix)(defaults, Object.assign({
    type
  }, option)));
  return interaction;
}
function initializeState(markState, options, library) {
  ...
  const framedStyle = frame ? (0, util_1.deepMix)({
    mainLineWidth: 1,
    mainStroke: '#000'
  }, style) : style;
  ...
  const uidScale = new Map(Array.from(markState.values()).flatMap(state => {
    ...
    return channels.map(({
      scale
    }) => [scale.uid, (0, scale_1.useRelationScale)(scale, library)]);
  }));
  ...
  for (const component of components) {
    ...
    for (const descriptor of scaleDescriptors) {
      ...
      (0, scale_1.assignScale)(scaleInstance, {
        [name]: scale
      });
    }
  ...
  for (const [mark, state] of markState.entries()) {
    ...
    const markScaleInstance = (0, array_1.mapObject)(scale, ({
      uid
    }) => uidScale.get(uid));
    ...
    const visualData = I.map((d, i) => {
      ...
      for (const [k, V] of Object.entries(value)) {
        datum[k] = V[d];
        if (S) datum[`series${(0, util_1.upperFirst)(k)}`] = S[i].map(i => V[i]);
      }
...
function plotView(view, selection, transitions, context) {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    selection.selectAll(className(constant_1.AREA_CLASS_NAME)).data(
    // Only render area with defined style.
    I.filter(i => (0, helper_1.defined)(areaStyles[i])), i => areaKeys[i]).join(enter => enter.append('rect').attr('className', constant_1.AREA_CLASS_NAME).style('zIndex', -2).call(styleArea), update => update.call(styleArea), exit => exit.remove());
    ...
    const componentsTransitions = selection.selectAll(className(constant_1.COMPONENT_CLASS_NAME)).data(components, d => `${d.type}-${d.position}-${d.index}`).join(enter => enter.append('g').style('zIndex', ({
      zIndex
    }) => zIndex || -1).attr('className', constant_1.COMPONENT_CLASS_NAME).append(options => (0, component_1.renderComponent)((0, util_1.deepMix)({
      ...
      if (preserve) return;
      const newComponent = (0, component_1.renderComponent)((0, util_1.deepMix)({
        animate: componentAnimateOptions,
        scale
    ...
    const T = selection.selectAll(className(constant_1.PLOT_CLASS_NAME)).data([layout], () => key).join(enter => enter
    // Make this layer interactive, such as click and mousemove events.
    .append('rect').style('zIndex', 0).style('fill', 'transparent').attr('className', constant_1.PLOT_CLASS_NAME).call(updateBBox).call(updateLayers, Array.from(markState.keys())).call(applyClip, clip), update => update.call(updateLayers, Array.from(markState.keys())).call(updateBBox)
    // .call((selection) => {
    ...
    for (const [mark, state] of markState.entries()) {
      ...
      const T = viewNode.selectAll(className(constant_1.ELEMENT_CLASS_NAME)).selectFacetAll(facetElements).data(data, d => d.key, d => d.groupKey).join(enter => enter.append(shapeFunction)
      // Note!!! Only one className can be set.
      // Using attribute as alternative for other classNames.
      .attr('className', constant_1.ELEMENT_CLASS_NAME).attr('markType', type).transition(function (data, i, element) {
        ...
        }).attr('markType', type).attr('className', constant_1.ELEMENT_CLASS_NAME);
      }), exit => {
      ...
      .append(shapeFunction).attr('className', constant_1.ELEMENT_CLASS_NAME).attr('markType', type).transition(function (data, i, element) {
        ...
        const toElements = enter.append(shapeFunction).attr('className', constant_1.ELEMENT_CLASS_NAME).attr('markType', type).nodes();
...
function plotLabel(view, selection, transitions, library, context) {
  ...
  const labels = Array.from(markState.entries()).flatMap(([mark, state]) => {
    ...
    const elements = selection.select(`#${key}`).selectAll(className(constant_1.ELEMENT_CLASS_NAME)).nodes()
  ...
  // Render all labels.
  const labelShapes = (0, selection_1.select)(labelLayer).selectAll(className(constant_1.LABEL_CLASS_NAME)).data(labels, d => d.key).join(enter => enter.append(d => labelShapeFunction.get(d)(d)).attr('className', constant_1.LABEL_CLASS_NAME), update => update.each(function (d, i, element) {
...
function plotBreak(view, selection, library, context) {
  ...
  // Render all breaks.
  (0, selection_1.select)(breakLayer).selectAll(className(constant_1.BREAK_CLASS_NAME)).data(breaks, d => d.key).join(enter => enter.append((d, index) => breaksShapeFunction.get(d)(d, index)).attr('className', constant_1.BREAK_CLASS_NAME), update => update.each(function (d, i, element) {
...
function createLabelShapeFunction(mark, state, view, library, context) {
  ...
  const shapeContext = Object.assign(Object.assign({}, context), {
    document: (0, library_1.documentOf)(context),
    theme,
    coordinate
  });
  return options => {
...
function valueOf(value, datum, i, data, options) {
  ...
  if ((0, helper_1.isStrictObject)(datum) && datum[value] !== undefined) return datum[value];
  return value;
}
/**
...
function applyTransform(node, context) {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    const [, {
      data: newData
    }] = yield (0, transform_1.applyDataTransform)([], {
      data
    }, context);
    return Object.assign({
      data: newData
...
function updateLayers(selection, marks) {
  ...
  selection.selectAll(className(constant_1.MAIN_LAYER_CLASS_NAME)).data(marks, d => d.key).join(enter => enter.append('g').attr('className', constant_1.MAIN_LAYER_CLASS_NAME).attr('id', d => d.key).style('facet', facet).style('fill', 'transparent').style('zIndex', d => {
    var _a;
  ...
  selection.append('g').attr('className', constant_1.LABEL_LAYER_CLASS_NAME).style('zIndex', 0);
}
