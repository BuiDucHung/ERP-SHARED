...
function getPyramidPoints(points, nextPoints, previousPoints, coordinate, reverse) {
  ...
  if ((0, coordinate_1.isTranspose)(coordinate)) {
    if (reverse) {
      const newP0 = [previousPoints ? previousPoints[1][0] : (p0[0] + p3[0]) / 2, p0[1]];
      const newP3 = [previousPoints ? previousPoints[2][0] : (p0[0] + p3[0]) / 2, p3[1]];
  ...
  if (reverse) {
    const newP0 = [p0[0], previousPoints ? previousPoints[1][1] : (p0[1] + p3[1]) / 2];
    const newP3 = [p3[0], previousPoints ? previousPoints[2][1] : (p0[1] + p3[1]) / 2];
    return [newP0, p1, p2, newP3];
  }
...
const Pyramid = (options, context) => {
  return (0, funnel_1.Funnel)(Object.assign({
