...
function selectG2Elements(root) {
  return (0, selection_1.select)(root).selectAll(`.${runtime_1.ELEMENT_CLASS_NAME}`).nodes().filter(d => !d.__removed__);
}
...
function renderLink(_a) {
  ...
  const append = elements => {
    ...
    for (let i = 1; i < sortedElements.length; i++) {
      ...
      p.lineTo(...p3);
      p.closePath();
      const _b = (0, array_1.mapObject)(style, d => valueof(d, e0)),
        {
...
function offsetTransform(element, offset, coordinate) {
  ...
  if ((0, coordinate_1.isTranspose)(coordinate)) return append(`translate(${offset}, 0)`);
...
function renderBackground(_a) {
  ...
  const sizeXOf = (element, padding) => {
    ...
    if (!(0, scale_1.isOrdinalScale)(scaleX)) return [0, 1];
    const {
      __data__: data
    } = element;
    const {
  ...
  const sizeYOf = (element, padding) => {
    ...
    if (!(0, scale_1.isOrdinalScale)(scaleY)) return [0, 1];
    const {
      __data__: data
    } = element;
    const {
  ...
  const bandShapeOf = (element, style) => {
    ...
    return (0, color_1.rect)(document, points, {
      y: dy,
      y1: dy1
    }, coordinate, style);
  };
  ...
  const append = element => {
    ...
    const _a = (0, array_1.mapObject)(rest, d => valueof(d, element)),
      {
        fill = '#CCD6EC',
        fillOpacity = 0.3,
        zIndex = -2,
 ...
 */
function createFindElementByEvent({
  elementsof,
  root,
  coordinate,
  scale,
  validFindByXMarks = exports.VALID_FIND_BY_X_MARKS
}) {
  var _a, _b;
    ...
    for (const [mark, state] of marks.entries()) {
      if (state === null || state === void 0 ? void 0 : state.channels) {
        ...
        for (const channel of state.channels) {
          ...
          } else if (channel && (channel.name === 'y' || channel.name.startsWith('y')) &&
          // Support y, y1, y2, y3, etc.
