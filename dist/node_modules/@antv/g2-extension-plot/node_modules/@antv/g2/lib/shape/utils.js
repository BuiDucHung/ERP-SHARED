...
function arrowPoints(from, to, options) {
  ...
  const size = typeof arrowSize === 'string' ? +parseFloat(arrowSize) / 100 * (0, vector_1.dist)(from, to) : arrowSize;
...
function getConnectStyle(style) {
  ...
  return Object.fromEntries(Object.entries(style).filter(([key]) => key.startsWith(PREFIX)).map(([key, value]) => [(0, util_1.lowerFirst)(key.replace(PREFIX, '').trim()), value]).filter(([key]) => key !== undefined));
}
...
function getTransform(coordinate, value) {
  if (!(0, coordinate_1.isPolar)(coordinate)) return '';
  const center = coordinate.getCenter();
  const {
    transform: suffix
  } = value;
