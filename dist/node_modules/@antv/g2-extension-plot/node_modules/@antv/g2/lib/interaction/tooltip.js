...
function heatmapItem(element) {
  ...
  if (hasCustomItems) {
    ...
    const newItems = items.filter(helper_1.defined).map(_a => {
      var {
...
function singleItem(element) {
  ...
  if ((0, helper_1.isHeatmap)(element)) return heatmapItem(element);
  const {
    title,
    items = []
  } = datum;
...
function groupItems(elements, scale, groupName, data = elements.map(d => d['__data__']), theme = {}) {
  ...
  const newItems = data.flatMap((datum, i) => {
    ...
    return definedItems.map(_a => {
      ...
      const name1 = useGroupName && !(transform_1.CALLBACK_ITEM_SYMBOL in item) ? groupName || name : name || groupName;
      return Object.assign(Object.assign({}, item), {
        color,
        name: name1 || title
      });
...
function equalBandWidth(scale) {
  ...
  if ((0, util_1.isNumber)(valueBandWidth)) return true;
...
function findSingleElement({
  ...
  const inAnnotation = d => constant_1.ANNOTATION_MARKS.includes(d.markType);
  const markEls = elements.filter(d => !inAnnotation(d));
  const isBar = markEls.length > 0 && markEls.every(inInterval) && !(0, coordinate_1.isPolar)(coordinate);
...
function findSeriesElement({
  ...
  for (const element of elements) {
    if (constant_1.ANNOTATION_MARKS.includes(element.markType)) continue;
    const {
      __data__: data
    } = element;
    const {
  ...
  const inInterval = d => d.markType === 'interval';
  const isBar = itemElements.length && itemElements.every(inInterval) && !(0, coordinate_1.isPolar)(coordinate);
  ...
  if (isBar) itemElements.sort((a, b) => xof(a) - xof(b));else {
...
function seriesTooltip(root, _a) {
  var {
    ...
    rest = __rest(_a, ["elements", "sort", "filter", "scale", "coordinate", "crosshairs", "crosshairsX", "crosshairsY", "render", "groupName", "emitter", "wait", "leading", "trailing", "startX", "startY", "body", "single", "position", "enterable", "mount", "bounding", "theme", "offset", "disableNative", "marker", "preserve", "style", "css", "clickLock", "disableAutoHide"]);
  ...
  const update = (0, util_1.throttle)(event => {
    ...
    emitter.emit('tooltip:show', Object.assign(Object.assign({}, event), {
      ...
      data: Object.assign(Object.assign({}, tooltipData), {
        data: {
          x: (0, scale_1.invert)(scale.x, transformedX, true)
        }
      })
    }));
  }, wait, {
  ...
  const onTooltipShow = _a => {
    ...
    const rootBounds = (0, utils_1.bboxOf)(root);
  ...
  const pointerleave = e => {
    if ((0, utils_1.mousePosition)(root, e)) return;
    hide(e);
  };
  ...
  const pointermove = (0, util_1.throttle)(event => {
    ...
    emitter.emit('tooltip:show', Object.assign(Object.assign({}, event), {
      ...
      data: Object.assign(Object.assign({}, data), {
        data: (0, helper_1.dataOf)(element, view)
      })
    }));
  }, wait, {
    leading,
...
function Tooltip(options) {
  ...
  return (target, viewInstances, emitter) => {
    ...
    return tooltip(plotArea, Object.assign(Object.assign({}, rest), {
      datum: (0, utils_1.createDatumof)(view),
      elements: utils_1.selectG2Elements,
      scale,
      coordinate,
      groupKey: shared ? (0, utils_1.createXKey)(view) : undefined,
      item,
      emitter,
      view,
      theme,
