...
exports.Advance = (0, createElement_1.createElement)(g => {
  ...
  const _b = (0, helper_1.subObject)(rest, 'background'),
    {
      padding
    } = _b,
    backgroundStyle = __rest(_b, ["padding"]);
  const _d = (0, helper_1.subObject)(rest, 'connector'),
    {
  ...
  let textShape;
  if (innerHTML) {
    textShape = (0, selection_1.select)(g).maybeAppend('html', 'html', className).style('zIndex', 0).style('innerHTML', innerHTML).call(utils_1.applyStyle, Object.assign({
      transform: labelTransform,
      transformOrigin: labelTransformOrigin
    }, rest)).node();
  } else {
    textShape = (0, selection_1.select)(g).maybeAppend('text', 'text').style('zIndex', 0).style('text', text).call(utils_1.applyStyle, Object.assign({
      textBaseline: 'middle',
      transform: labelTransform,
      transformOrigin: labelTransformOrigin
    }, rest)).node();
  }
  const rect = (0, selection_1.select)(g).maybeAppend('background', 'rect').style('zIndex', -1).call(utils_1.applyStyle, inferBackgroundBounds(textShape, padding)).call(utils_1.applyStyle, background ? backgroundStyle : {}).node();
  ...
  const markerStart = startMarker && new component_1.Marker({
    id: 'startMarker',
    style: Object.assign({
      x: 0,
      y: 0
    }, (0, helper_1.subObject)(rest, 'startMarker'))
  });
  const markerEnd = endMarker && new component_1.Marker({
    id: 'endMarker',
    style: Object.assign({
      x: 0,
      y: 0
    }, (0, helper_1.subObject)(rest, 'endMarker'))
  });
  (0, selection_1.select)(g).maybeAppend('connector', 'path').style('zIndex', 0).style('d', connectorPath).style('markerStart', markerStart).style('markerEnd', markerEnd).call(utils_1.applyStyle, connector ? connectorStyle : {});
});