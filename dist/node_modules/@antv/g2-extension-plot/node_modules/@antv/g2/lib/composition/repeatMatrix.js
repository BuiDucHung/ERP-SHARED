    ...
    return index.flatMap(i => {
      ...
      return children.map(d => {
        ...
        return Object.assign({
          ...
          paddingBottom: 0,
          frame: true,
          scale: (0, util_1.deepMix)(defaultScale, scale),
          axis: (0, util_1.deepMix)(defaultAxis, axis, newAxis),
          // Hide all legends for child mark by default,
          // they are displayed in the top-level.
          legend: false,
          encode: (0, util_1.deepMix)({}, encode, {
            x: fx,
            y: fy
          }),
...
function createGuideX(guideX) {
  ...
  return (facet, data) => {
    ...
    if (rowIndex !== rowValuesLength - 1) return (0, facetRect_1.createInnerGuide)(guideX, data);
  };
}
function createGuideY(guideY) {
  ...
  return (facet, data) => {
    ...
    if (columnIndex !== 0) return (0, facetRect_1.createInnerGuide)(guideY, data);
  };
}
/**
...
const RepeatMatrix = () => {
  return options => {
    const newOptions = container_1.Container.of(options).call(facetRect_1.toCell).call(facetRect_1.inferColor).call(setChildren).call(setData).call(facetRect_1.setAnimation).call(facetRect_1.setStyle).call(setScale).value();
    return [newOptions];
  };
};
