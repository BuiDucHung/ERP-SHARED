...
function groupNameOf(scale, datum) {
  ...
  const invertAble = scale => {
    return scale && scale.invert && !(scale instanceof scale_1.Band) && !(scale instanceof scale_1.Constant);
  };
  ...
  if (series && scaleSeries instanceof scale_1.Band && scaleSeries.invert(series) !== color && !facet) {
...
function maybeSubObject(obj, prefix) {
  const entries = Object.entries(obj || {}).filter(([key]) => key.startsWith(prefix)).map(([key, value]) => [(0, util_1.lowerFirst)(key.replace(prefix, '').trim()), value]).filter(([key]) => !!key);
