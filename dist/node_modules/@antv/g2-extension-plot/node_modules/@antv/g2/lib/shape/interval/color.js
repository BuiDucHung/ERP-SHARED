...
function rect(document, points, value, coordinate, style = {}) {
  ...
  if (!(0, coordinate_1.isPolar)(coordinate) && !(0, coordinate_1.isHelix)(coordinate)) {
    ...
    const clampWidth = tpShape ? (0, number_1.clamp)(finalWidth, minHeight, Infinity) : (0, number_1.clamp)(finalWidth, minWidth, maxWidth);
    const clampHeight = tpShape ? (0, number_1.clamp)(finalHeight, minWidth, maxWidth) : (0, number_1.clamp)(finalHeight, minHeight, Infinity);
    ...
    return (0, selection_1.select)(document.createElement('rect', {})).style('x', clampX).style('y', clampY).style('width', clampWidth).style('height', clampHeight).style('radius', [radiusTopLeft, radiusTopRight, radiusBottomRight, radiusBottomLeft]).call(utils_1.applyStyle, rest).node();
  }
  ...
  const path = (0, d3_shape_1.arc)().cornerRadius(radius).padAngle(inset * Math.PI / 180);
  return (0, selection_1.select)(document.createElement('path', {})).style('d', path(arcObject)).style('transform', `translate(${center[0]}, ${center[1]})`).style('radius', radius).style('inset', inset).call(utils_1.applyStyle, rest).node();
}
/**
 * Render rect in different coordinate.
...
const Color = (options, context) => {
  ...
  return (points, value, defaults) => {
    ...
    return (0, selection_1.select)(rect(document, points, value, coordinate, extendedStyle)).call(utils_1.applyStyle, restDefaults).style('fill', 'transparent').style(colorAttribute, color).style((0, utils_1.toOpacityKey)(options), opacity).style('lineWidth', lineWidth).style('stroke', stroke === undefined ? color : stroke)
    // shape.style has higher priority.
    .call(utils_1.applyStyle, rest).node();
  };
};
exports.Color = Color;
