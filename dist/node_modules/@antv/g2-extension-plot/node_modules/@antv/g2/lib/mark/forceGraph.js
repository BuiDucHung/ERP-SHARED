...
function dataTransform(data, layout, encode) {
  ...
  const simulation = (0, d3_force_1.forceSimulation)(nodes).force('link', linkForce).force('charge', nodeForce);
  joint ? simulation.force('center', (0, d3_force_1.forceCenter)()) : simulation.force('x', (0, d3_force_1.forceX)()).force('y', (0, d3_force_1.forceY)());
  simulation.stop();
...
const ForceGraph = options => {
  ...
  const {
    links,
    nodes
  } = (0, utils_1.initializeData)(data, encode);
  const {
    nodesData,
    linksData
  } = dataTransform({
    links,
    nodes
  }, (0, util_1.deepMix)({}, DEFAULT_LAYOUT_OPTIONS, layout), encode);
  const linkTooltip = (0, mark_1.subTooltip)(tooltip, 'link', {
    items: [d => ({
      name: 'source',
      value: (0, utils_1.field)(linkKey)(d.source)
    }), d => ({
      name: 'target',
      value: (0, utils_1.field)(linkKey)(d.target)
    })]
  });
  const nodeTooltip = (0, mark_1.subTooltip)(tooltip, 'node', {
    items: [d => ({
      name: 'key',
      value: (0, utils_1.field)(nodeKey)(d)
    })]
  }, true);
  return [(0, util_1.deepMix)({}, DEFAULT_LINK_OPTIONS, {
    data: linksData,
    encode: linkEncode,
    labels: linkLabels,
    ...
    tooltip: nodeTooltip,
    labels: [Object.assign(Object.assign({}, DEFAULT_LABEL_OPTIONS), (0, helper_1.subObject)(style, 'label')), ...nodeLabels],
    animate: (0, mark_1.maybeAnimation)(animate, 'link')
  })];
};
