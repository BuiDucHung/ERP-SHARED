...
function getNodeAlignFunction(nodeAlign) {
  ...
  if (type === 'string') return ALIGN_METHOD[nodeAlign] || d3_sankey_1.justify;
...
const Sankey = options => {
  return data => {
    ...
    const sankeyProcessor = (0, d3_sankey_1.sankey)().nodeSort(nodeSort).linkSort(linkSort).links(nodeLinks).nodes(nodeNodes).nodeWidth(nodeWidth).nodePadding(nodePadding).nodeDepth(nodeDepth).nodeAlign(getNodeAlignFunction(nodeAlign)).iterations(iterations).extent([[0, 0], [1, 1]]);
