...
function getPath(points, coordinate) {
  if (!(0, coordinate_1.isPolar)(coordinate)) return (0, d3_shape_1.line)().x(d => d[0]).y(d => d[1])(points);
...
function getTransform(coordinate, transform) {
  if (!(0, coordinate_1.isPolar)(coordinate)) return transform;
...
const Line = (options, context) => {
  ...
  return (points, value, defaults) => {
    ...
    }, (0, helper_1.subObject)(style, 'arrow'))) : null;
    const path = getPath(points, coordinate);
    const transform = getTransform(coordinate, value.transform);
    return (0, selection_1.select)(document.createElement('path', {})).call(utils_1.applyStyle, shapeTheme).style('d', path).style('stroke', color).style('lineWidth', size).style('transform', transform).style('markerEnd', arrowMarker).call(utils_1.applyStyle, style).node();
  };
};
exports.Line = Line;
exports.Line.props = {
