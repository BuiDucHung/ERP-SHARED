...
const Color = (options, context) => {
  ...
  return (points, value, defaults) => {
    ...
    return (0, selection_1.select)(document.createElement('path', {})).call(utils_1.applyStyle, defaults).style('fill', 'transparent').style('d', path(cx, cy, finalRadius)).style('lineWidth', finalLineWidth).style('transform', transform).style('transformOrigin', `${cx - finalRadius} ${cy - finalRadius}`).style('stroke', color).style((0, utils_1.toOpacityKey)(options), opacity).style(colorAttribute, color).call(utils_1.applyStyle, style).node();
  };
};
exports.Color = Color;
exports.Color.props = {
