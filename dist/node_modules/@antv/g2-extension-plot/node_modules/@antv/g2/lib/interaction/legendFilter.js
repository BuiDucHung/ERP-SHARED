...
function legendFilterOrdinal(root, {
  ...
  const markerStyle = {
    unselected: (0, helper_1.subObject)(unselected, 'marker')
  };
  const labelStyle = {
    unselected: (0, helper_1.subObject)(unselected, 'label')
  };
  const {
    setState: setM,
    removeState: removeM
  } = (0, utils_1.useState)(markerStyle, undefined);
  const {
    setState: setL,
    removeState: removeL
  } = (0, utils_1.useState)(labelStyle, undefined);
...
function legendFilterOrdinalHtml(root, {
  domain,
  filter,
  defaultSelect,
  emitter,
  ...
  // View instance,
  {
    ...
    setState(legend, viewOptions => {
      ...
      const newMarks = marks.map(mark => {
        ...
        if (constant_1.ANNOTATION_MARKS.includes(mark.type)) return mark;
        ...
        return (0, util_1.deepMix)({}, mark, Object.assign(Object.assign({
...
function LegendFilter() {
  return (context, contexts, emitter) => {
    ...
    const legends = [...legendsOf(container), ...legendsHtmlOf(container), ...legendsContinuousOf(container)];
    const allChannels = legends.flatMap(channelsOf);
    const filter = isFacet ? (0, util_1.throttle)(filterFacets, 50, {
      trailing: true
    }) : (0, util_1.throttle)(filterView, 50, {
      trailing: true
    });
