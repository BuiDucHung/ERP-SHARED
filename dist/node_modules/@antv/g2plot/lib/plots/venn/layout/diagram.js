...
function circleMargin(current, interior, exterior) {
  var margin = interior[0].radius - (0, circleintersection_1.distance)(interior[0], current),
    i,
    m;
  for (i = 1; i < interior.length; ++i) {
    m = interior[i].radius - (0, circleintersection_1.distance)(interior[i], current);
    if (m <= margin) {
      margin = m;
    }
  }
  for (i = 0; i < exterior.length; ++i) {
    m = (0, circleintersection_1.distance)(exterior[i], current) - exterior[i].radius;
    if (m <= margin) {
      margin = m;
    }
  }
...
function computeTextCentre(interior, exterior) {
  ...
  var solution = (0, fmin_1.nelderMead)(function (p) {
  ...
  var valid = true;
  for (i = 0; i < interior.length; ++i) {
    if ((0, circleintersection_1.distance)(ret, interior[i]) > interior[i].radius) {
      valid = false;
      break;
    }
  }
  for (i = 0; i < exterior.length; ++i) {
    if ((0, circleintersection_1.distance)(ret, exterior[i]) < exterior[i].radius) {
      valid = false;
      break;
    }
  }
  if (!valid) {
    if (interior.length == 1) {
      ...
      var areaStats = {};
      (0, circleintersection_1.intersectionArea)(interior, areaStats);
      if (areaStats.arcs.length === 0) {
        ...
        ret = (0, circleintersection_1.getCenter)(areaStats.arcs.map(function (a) {
          return a.p1;
        }));
      }
    }
  ...
  for (var i = 0; i < circleids.length; i++) {
    ...
    for (var j = i + 1; j < circleids.length; ++j) {
      var b = circles[circleids[j]],
        d = (0, circleintersection_1.distance)(a, b);
