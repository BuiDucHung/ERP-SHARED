...
function colorMap(params, data, colorPalette) {
  ...
  if (!(0, util_1.isArray)(palette)) {
    palette = data.filter(function (d) {
      return d[setsField].length === 1;
    }).length <= 10 ? colors10 : colors20;
  }
  var map = (0, utils_2.getColorMap)(palette, data, blendMode, setsField);
  return function (id) {
...
function transformColor(params, data) {
  ...
  if (typeof color !== 'function') {
    ...
    return function (datum) {
      return map_1(datum[constant_1.ID_FIELD]);
    };
  }
  return color;
}
...
function padding(params) {
  ...
  var tempPadding = (0, utils_1.normalPadding)(appendPadding);
  if (legend !== false) {
    tempPadding = (0, utils_1.getAdjustAppendPadding)(appendPadding, (0, util_1.get)(legend, 'position'), exports.LEGEND_SPACE);
  }
  chart.appendPadding = (0, utils_1.resolveAllPadding)([tempPadding, padding]);
  return params;
}
/**
 * 处理非法数据
...
function data(params) {
  ...
  var filterSets = data.filter(function (datum) {
    var sets = datum.sets;
    // 存在非法元素，就过滤这条数据
    return (0, utils_2.islegalSets)(currSets, sets);
  });
  if (!(0, util_1.isEqual)(filterSets, data)) (0, utils_1.log)(utils_1.LEVEL.WARN, false, 'warn: %s', '交集中不能出现不存在的集合, 请输入合法数据');
  return (0, util_1.deepMix)({}, params, {
    options: {
      data: filterSets
    }
  });
...
function geometry(params) {
  ...
  // 获取容器大小
  var _a = (0, utils_1.normalPadding)(chart.appendPadding),
    t = _a[0],
    r = _a[1],
    b = _a[2],
    l = _a[3];
  ...
  var vennData = (0, utils_2.layoutVennData)(options, Math.max(width - (r + l), 0), Math.max(height - (t + b), 0), 0);
  chart.data(vennData);
  ...
  if (typeof colorOptions === 'function') {
    geometry.color(constant_1.ID_FIELD, function (id) {
      var datum = vennData.find(function (d) {
        return d[constant_1.ID_FIELD] === id;
      });
...
function label(params) {
  ...
  var geometry = (0, utils_1.findGeometry)(chart, 'schema');
  if (!label) {
    ...
    geometry.label({
      fields: ['id'],
      callback: callback,
      cfg: (0, util_1.deepMix)({}, (0, utils_1.transformLabel)(cfg), {
...
function vennInteraction(params) {
  ...
  if (interactions) {
    ...
    (0, common_1.interaction)((0, utils_1.deepAssign)({}, params, {
      options: {
        interactions: interactions.map(function (i) {
          return tslib_1.__assign(tslib_1.__assign({}, i), {
...
function adaptor(params) {
  ...
  return (0, utils_1.flow)(padding, common_1.theme, data, geometry, label, (0, common_1.scale)({}), legend, axis, common_1.tooltip, vennInteraction, common_1.animation
  // ... 其他的 adaptor flow
  )(params);
}
