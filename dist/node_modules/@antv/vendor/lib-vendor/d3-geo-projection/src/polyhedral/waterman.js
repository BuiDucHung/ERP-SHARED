...
function _default(faceProjection) {
  faceProjection = faceProjection || function (face) {
    var c = face.length === 6 ? (0, _index.geoCentroid)({
      type: "MultiPoint",
      coordinates: face
    }) : face[0];
    return (0, _index.geoGnomonic)().scale(1).translate([0, 0]).rotate([-c[0], -c[1]]);
  };
  ...
  w5.forEach(function (hexagon, j) {
    var _$face = _octahedron.default[j],
      n = _$face.length,
      normals = cornerNormals[j] = [];
    for (var i = 0; i < n; ++i) {
      w5.push([_$face[i], hexagon[(i * 2 + 2) % (2 * n)], hexagon[(i * 2 + 1) % (2 * n)]]);
      parents.push(j);
  ...
  return (0, _index1.default)(faces[0], face).angle(-30).scale(110.625).center([0, 45]);
}
function dot(a, b) {
