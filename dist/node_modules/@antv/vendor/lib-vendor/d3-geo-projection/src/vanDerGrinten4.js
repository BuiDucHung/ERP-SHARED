...
_export(exports, {
  ...
  vanDerGrinten4Raw: function () {
    return vanDerGrinten4Raw;
  }
});
...
vanDerGrinten4Raw.invert = function (x, y) {
  var delta;
  if (!x || !y) return [x, y];
  var sy = (0, _math.sign)(y);
  y = (0, _math.abs)(y) / _math.pi;
  var x1 = (0, _math.sign)(x) * x / _math.halfPi,
    D = (x1 * x1 - 1 + 4 * y * y) / (0, _math.abs)(x1),
    D2 = D * D,
    B = y * (2 - (y > 0.5 ? (0, _math.min)(y, (0, _math.abs)(x)) : 0)),
    r = x * x + y * y,
    i = 50;
  do {
    var B2 = B * B,
    ...
    B -= delta = f / f_;
  } while (delta * r * r > _math.epsilon && --i > 0);
  return [(0, _math.sign)(x) * ((0, _math.sqrt)(D * D + 4) + D) * _math.pi / 4, sy * _math.halfPi * B];
};
function _default() {
  return (0, _index.geoProjection)(vanDerGrinten4Raw).scale(127.16);
}