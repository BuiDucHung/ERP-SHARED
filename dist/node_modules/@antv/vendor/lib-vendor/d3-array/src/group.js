    ...
    groups = groups.flatMap(function (g) {
      return g.pop().map(function (param) {
        var _param = _sliced_to_array(param, 2),
          key = _param[0],
          value = _param[1];
        return _to_consumable_array(g).concat([key, value]);
      });
    });
  }
  return groups;
...
function flatGroup(values) {
  ...
  return flatten(groups.apply(void 0, [values].concat(_to_consumable_array(keys))), keys);
}
function flatRollup(values, reduce) {
  for (var _len = arguments.length, keys = new Array(_len > 2 ? _len - 2 : 0), _key = 2; _key < _len; _key++) {
    keys[_key - 2] = arguments[_key];
  }
  return flatten(rollups.apply(void 0, [values, reduce].concat(_to_consumable_array(keys))), keys);
}
