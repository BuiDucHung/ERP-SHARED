...
function renderStraight(data, attr) {
  ...
  return data.map(function (_a, index) {
    var value = _a.value;
    var _b = tslib_1.__read((0, line_1.getValuePos)(value, attr), 2),
      x = _b[0],
      y = _b[1];
    var _c = tslib_1.__read((0, util_2.scale)(getGridVector(value, attr), gridLength), 2),
      dx = _c[0],
      dy = _c[1];
    return {
      id: index,
...
function renderSurround(data, attr) {
  ...
  var _a = tslib_1.__read(center, 2),
    cx = _a[0],
    cy = _a[1];
  return data.map(function (_a, index) {
    var value = _a.value;
    var _b = tslib_1.__read((0, line_1.getValuePos)(value, attr), 2),
      sx = _b[0],
      sy = _b[1];
    var _c = tslib_1.__read([sx - cx, sy - cy], 2),
      dx = _c[0],
      dy = _c[1];
    var points = [];
    controlAngles.forEach(function (angle) {
      var angleInRad = (0, util_2.degToRad)(angle);
      var _a = tslib_1.__read([Math.cos(angleInRad), Math.sin(angleInRad)], 2),
        cosa = _a[0],
        sina = _a[1];
...
function renderGrid(container, data, attr, animate) {
  ...
  var style = tslib_1.__assign(tslib_1.__assign({}, gridAttr), {
    center: center,
    areaFill: (0, util_1.isFunction)(areaFill) ? finalData.map(function (datum, index) {
      return (0, util_2.getCallbackValue)(areaFill, [datum, index, finalData]);
    }) : areaFill,
    animate: animate,
    data: gridItems
  });
  return container.selectAll(constant_1.CLASS_NAMES.grid.class).data([1]).join(function (enter) {
    var grid = enter.append(function () {
      return new grid_1.Grid({
        style: style
      });
    }).attr('className', constant_1.CLASS_NAMES.grid.name);
    (0, classname_1.applyClassName)(grid, constant_1.CLASS_NAMES.grid, classname_map_1.CLASSNAME_SUFFIX_MAP.grid, classNamePrefix);
    return grid;
  }, function (update) {
