...
function applyStyleSheet(element, style) {
  Object.entries(style).forEach(function (_a) {
    ...
    tslib_1.__spreadArray([element], tslib_1.__read(element.querySelectorAll(selector)), false).filter(function (el) {
...
function subStyleProps(style, prefix, invert) {
  ...
  Object.entries(style).forEach(function (_a) {
    ...
    // @example showHandle -> showHandle, showHandleLabel -> showLabel
      ...
      if (key === (0, string_1.addPrefix)(prefix, 'show')) result[key] = value;else result[key.replace(new RegExp((0, string_1.toUppercaseFirstLetter)(prefix)), '')] = value;
    }
...
function superStyleProps(style, prefix) {
  return Object.entries(style).reduce(function (acc, _a) {
    var _b = tslib_1.__read(_a, 2),
      key = _b[0],
      value = _b[1];
    if (key.startsWith('show')) acc["show".concat(prefix).concat(key.slice(4))] = value;else acc["".concat(prefix).concat((0, string_1.toUppercaseFirstLetter)(key))] = value;
    return acc;
  }, {});
}
/**
