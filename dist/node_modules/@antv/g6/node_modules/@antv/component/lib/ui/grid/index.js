...
function getSurroundPath(points, attr, reversed) {
  ...
  return points.reduce(function (r, p, idx) {
    if (idx === 0) r.push(tslib_1.__spreadArray(['M'], tslib_1.__read(p), false));else r.push(tslib_1.__spreadArray(['A', radius, radius, 0, 0, sweepFlag], tslib_1.__read(p), false));
    return r;
  }, []);
}
...
function connectPaths(from, to, cfg) {
  ...
  var _a = tslib_1.__read([getLinePath(from, cfg), getLinePath(to.slice().reverse(), cfg, true)], 2),
    path1 = _a[0],
    path2 = _a[1];
  var _b = tslib_1.__read([from[0], to.slice(-1)[0]], 2),
    startOfFrom = _b[0],
    endOfTo = _b[1];
  ...
  var _c = tslib_1.__read([(0, util_1.distance)(endOfTo, center), (0, util_1.distance)(startOfFrom, center)], 2),
    raduis1 = _c[0],
    radius2 = _c[1];
  return createPath([tslib_1.__spreadArray(['A', raduis1, raduis1, 0, 0, 1], tslib_1.__read(endOfTo), false), tslib_1.__spreadArray(['L'], tslib_1.__read(endOfTo), false)], [tslib_1.__spreadArray(['A', radius2, radius2, 0, 0, 0], tslib_1.__read(startOfFrom), false), tslib_1.__spreadArray(['L'], tslib_1.__read(startOfFrom), false)]);
}
function renderGridLine(container, data, attr, style) {
  ...
  return container.selectAll(CLASS_NAMES.line.class).data(lines, function (d) {
    ...
    return enter.append('path').each(function (datum, index) {
      var lineStyle = (0, util_1.getCallbackValue)((0, util_1.getPrimitiveAttributes)(tslib_1.__assign({
        d: datum.d
      }, style)), [datum, index, lines]);
      this.attr(tslib_1.__assign({
...
function renderAlternateRegion(container, data, style) {
  ...
  for (var idx = 0; idx < data.length - 1; idx++) {
    var _a = tslib_1.__read([data[idx].points, data[idx + 1].points], 2),
      prev = _a[0],
      curr = _a[1];
...
function getData(attr) {
  ...
  return data.map(function (datum) {
    ...
    var _a = tslib_1.__read(points, 1),
      start = _a[0];
    return tslib_1.__assign(tslib_1.__assign({}, datum), {
      points: tslib_1.__spreadArray(tslib_1.__spreadArray([], tslib_1.__read(points), false), [start], false)
    });
  });
}
var Grid = /** @class */function (_super) {
  ...
  Grid.prototype.render = function (attributes, container) {
    ...
    var type = attributes.type,
      ...
      style = tslib_1.__rest(attributes, ["type", "center", "areaFill", "closed"]);
