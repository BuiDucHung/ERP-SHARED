...
var Tag = /** @class */function (_super) {
  ...
  Tag.prototype.render = function (attributes, container) {
    ...
    var _b = tslib_1.__read((0, util_2.parseSeriesAttr)(padding), 4),
      ...
      pb = _b[2],
      pl = _b[3];
    var group = (0, util_2.maybeAppend)(container, '.tag-content', 'g').attr('className', 'tag-content').style('zIndex', 0).node();
    var markerStyle = marker || {
      symbol: 'triangle',
      size: 0
    };
    var markerShape = (0, util_2.maybeAppend)(group, '.tag-marker', function () {
    ...
    var _c = getTextPosition(markerShape, spacing),
      x = _c.x,
      y = _c.y;
    (0, util_2.select)(group).maybeAppendByClassName('tag-text', function () {
      return new shapes_1.Text();
    }).styles(tslib_1.__assign(tslib_1.__assign({
      fontSize: 12,
      text: (0, util_1.isNil)(text) ? '' : "".concat(text),
      x: x,
      y: y
    }, labelStyle), {
      textBaseline: 'middle'
    ...
    (0, util_2.select)(container).maybeAppendByClassName('tag-background', 'rect').styles(tslib_1.__assign({
      zIndex: -1,
      y: bounds.min[1] - pt,
      x: bounds.min[0] - pl,
