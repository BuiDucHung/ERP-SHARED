...
var HandleIcon = /** @class */function (_super) {
  ...
  HandleIcon.prototype.render = function (attributes, container) {
    var x = attributes.x,
      ...
      iconStyle = tslib_1.__rest(attributes, ["x", "y", "size", "radius", "orientation", "classNamePrefix"]);
    // default handle
    var width = size;
    ...
    var rect = (0, util_1.select)(container).maybeAppendByClassName(CLASS_NAMES.iconRect, 'rect').attr('className', rectClassName).styles(tslib_1.__assign(tslib_1.__assign({}, iconStyle), {
      width: width,
      height: height,
      radius: radius,
      x: x - width / 2,
    ...
    rect.maybeAppendByClassName("".concat(CLASS_NAMES.iconLine, "-1"), 'line').attr('className', lineClassName(1)).styles(tslib_1.__assign({
      x1: x1,
      x2: x1,
      y1: y1,
      y2: y2
    }, iconStyle));
    rect.maybeAppendByClassName("".concat(CLASS_NAMES.iconLine, "-2"), 'line').attr('className', lineClassName(2)).styles(tslib_1.__assign({
      x1: x2,
      x2: x2,
      y1: y1,
      y2: y2
...
var Handle = /** @class */function (_super) {
  ...
  Handle.prototype.renderLabel = function (container) {
    ...
    var _b = (0, util_1.subStyleProps)(this.attributes, 'label'),
      _c = _b.x,
      ...
      style = tslib_1.__rest(_b, ["x", "y", "transform", "transformOrigin"]);
    var _e = tslib_1.__read((0, util_1.splitStyle)(style, []), 2),
      labelStyle = _e[0],
      groupStyle = _e[1];
    var labelGroup = (0, util_1.select)(container).maybeAppendByClassName(CLASS_NAMES.labelGroup, 'g').styles(groupStyle);
    // @ts-ignore
    var _f = tslib_1.__assign(tslib_1.__assign({}, constant_1.HANDLE_LABEL_DEFAULT_CFG), labelStyle),
      text = _f.text,
      rest = tslib_1.__rest(_f, ["text"]);
    (0, util_1.ifShow)(!!showLabel, labelGroup, function (group) {
      _this.label = group.maybeAppendByClassName(CLASS_NAMES.label, 'text').styles(tslib_1.__assign(tslib_1.__assign({}, rest), {
        x: x + labelX,
        y: y + labelY,
        transform: transform,
  ...
  Handle.prototype.renderIcon = function (container) {
    ...
    var iconStyle = tslib_1.__assign(tslib_1.__assign({
      x: x,
      y: y,
      orientation: orientation,
      classNamePrefix: classNamePrefix
    }, constant_1.HANDLE_ICON_DEFAULT_CFG), (0, util_1.subStyleProps)(this.attributes, 'icon'));
