...
function getTickPoints(unitVector, tickLength) {
  var _a = tslib_1.__read(unitVector, 2),
    dx = _a[0],
    dy = _a[1];
...
function getTickLineLayout(datum, index, data, tickVector, attr) {
  ...
  var _a = tslib_1.__read(getTickPoints(tickVector, (0, util_2.getCallbackValue)(tickLength, [datum, index, data])), 2),
...
function createTickEl(container, datum, index, data, attr) {
  ...
  var el = 'line';
  if ((0, util_1.isFunction)(formatter)) el = function () {
    return (0, util_2.getCallbackValue)(formatter, [datum, index, data, tickVector]);
  };
  var tick = container.append(el).attr('className', constant_1.CLASS_NAMES.tickItem.name);
  (0, classname_1.applyClassName)(tick, constant_1.CLASS_NAMES.tickItem, classname_map_1.CLASSNAME_SUFFIX_MAP.tickItem, classNamePrefix);
  return tick;
}
function applyTickStyle(datum, index, data, tick, group, attr, style) {
  ...
  var _a = getTickLineLayout(datum, index, data, tickVector, attr),
    ...
    y1 = _a.y1,
    y2 = _a.y2;
  var _b = tslib_1.__read((0, util_2.splitStyle)((0, utils_1.getCallbackStyle)(style, [datum, index, data, tickVector])), 2),
    tickStyle = _b[0],
    groupStyle = _b[1];
  tick.node().nodeName === 'line' && tick.styles(tslib_1.__assign({
    x1: x1,
    x2: x2,
    y1: y1,
    y2: y2
...
function createTick(datum, index, data, attr, tickAttr, animate) {
  ...
  var _a = tslib_1.__read((0, line_1.getValuePos)(datum.value, attr), 2),
    x = _a[0],
    y = _a[1];
  return (0, animation_1.transition)(this, {
...
function renderTicks(container, axisData, attr, animate) {
  ...
  return container.selectAll(constant_1.CLASS_NAMES.tick.class).data(finalData, function (d) {
    return d.id || d.label;
  }).join(function (enter) {
    var ticks = enter.append('g').attr('className', constant_1.CLASS_NAMES.tick.name).transition(function (datum, index) {
      return createTick.call(this, datum, index, finalData, attr, tickAttr, false);
    });
    (0, classname_1.applyClassName)(ticks, constant_1.CLASS_NAMES.tick, classname_map_1.CLASSNAME_SUFFIX_MAP.tick, classNamePrefix);
    return ticks;
  }, function (update) {
