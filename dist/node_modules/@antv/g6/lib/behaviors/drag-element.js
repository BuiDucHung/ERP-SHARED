...
class DragElement extends base_behavior_1.BaseBehavior {
  constructor(context, options) {
    ...
    this.setCursor = event => {
      ...
      if (type === constants_1.CommonEvent.POINTER_ENTER) canvas.setCursor((cursor === null || cursor === void 0 ? void 0 : cursor.grab) || 'grab');else canvas.setCursor((cursor === null || cursor === void 0 ? void 0 : cursor.default) || 'default');
    };
  ...
  isKeydown() {
    ...
    if (!(trigger === null || trigger === void 0 ? void 0 : trigger.length)) return true;
   ...
   */
  validate(event) {
    if (this.destroyed || (0, element_1.isToBeDestroyed)(event.target) ||
    // @ts-expect-error private property
    // 避免动画冲突，在combo/node折叠展开过程中不触发
    this.context.graph.isCollapsingExpanding || !this.isKeydown()) return false;
    const {
      enable
    } = this.options;
  ...
  hideEdge() {
    ...
    if (hideEdge === 'all') this.hiddenEdges = graph.getEdgeData().map(id_1.idOf);else {
