...
class DragElementForce extends drag_element_1.DragElement {
  ...
  moveElement(ids, offset) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      this.context.graph.getNodeData(ids).forEach((element, index) => {
        ...
        if (layout) (0, layout_1.invokeLayoutMethod)(layout, 'setFixedPosition', ids[index], [...(0, vector_1.add)([+x, +y], this.clampByRotation(offset))]);
      });
    });
  }
  /**
  ...
  onDragStart(event) {
    ...
    const layout = this.forceLayoutInstance;
    if (layout) (0, layout_1.getLayoutProperty)(layout, 'simulation').alphaTarget(0.3).restart();
    this.context.graph.getNodeData(this.target).forEach(element => {
      const {
        x = 0,
        y = 0
      } = element.style || {};
      if (layout) (0, layout_1.invokeLayoutMethod)(layout, 'setFixedPosition', (0, id_1.idOf)(element), [+x, +y]);
    });
  }
  /**
  ...
  onDragEnd() {
    const layout = this.forceLayoutInstance;
    if (layout) (0, layout_1.getLayoutProperty)(layout, 'simulation').alphaTarget(0);
    if (this.options.fixed) return;
    this.context.graph.getNodeData(this.target).forEach(element => {
      if (layout) (0, layout_1.invokeLayoutMethod)(layout, 'setFixedPosition', (0, id_1.idOf)(element), [null, null, null]);
    });
  }
}
