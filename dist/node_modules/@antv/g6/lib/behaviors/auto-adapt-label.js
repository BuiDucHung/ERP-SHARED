...
class AutoAdaptLabel extends base_behavior_1.BaseBehavior {
  constructor(context, options) {
    ...
    this.hideLabelIfExceedViewport = (prevElementsInView, currentElementsInView) => {
      const {
        exit
      } = (0, diff_1.arrayDiff)(prevElementsInView, currentElementsInView, d => d.id);
    ...
    this.sortLabelElementsInView = labelElements => {
      ...
      if ((0, util_1.isFunction)(sort)) return labelElements.sort((a, b) => sort(model.getElementDataById(a.id), model.getElementDataById(b.id)));
      const {
        node: nodes = [],
        edge: edges = [],
        combo: combos = []
      } = (0, util_1.groupBy)(labelElements, el => el.type);
      const sortedCombos = (0, util_1.isFunction)(sortCombo) ? combos.sort((a, b) => sortCombo(...model.getComboData([a.id, b.id]))) : combos;
      const sortedNodes = (0, util_1.isFunction)(sortNode) ? nodes.sort((a, b) => sortNode(...model.getNodeData([a.id, b.id]))) : this.sortNodesByCentrality(nodes, sortNode);
      const sortedEdges = (0, util_1.isFunction)(sortEdge) ? edges.sort((a, b) => sortEdge(...model.getEdgeData([a.id, b.id]))) : edges;
    ...
    this.hideLabel = element => {
      ...
      if (label) (0, visibility_1.setVisibility)(label, 'hidden');
      this.hiddenElements.set(element.id, element);
    };
    this.showLabel = element => {
      const label = element.getShape('label');
      if (label) (0, visibility_1.setVisibility)(label, 'visible');
      element.toFront();
      this.hiddenElements.delete(element.id);
    };
    this.onTransform = (0, util_1.throttle)(this.onToggleVisibility, this.options.throttle, {
      leading: true
    });
  ...
  validate(event) {
    ...
    if ((0, util_1.isFunction)(enable)) return enable(event);
    return !!enable;
  }
  destroy() {
    this.unbindEvents();
