...
class CollapseExpandNode extends base_transform_1.BaseTransform {
  ...
  handleExpand(node, input) {
    ...
    if ((0, collapsibility_1.isCollapsed)(node)) return;
  ...
  beforeDraw(input) {
    ...
    if (!model.model.hasTreeStructure(constants_1.TREE_KEY)) return input;
    const {
      add: {
        nodes: nodesToAdd,
        edges: edgesToAdd
    ...
    nodesToAdd.forEach((node, id) => {
      if ((0, collapsibility_1.isCollapsed)(node)) nodesToCollapse.set(id, node);
    });
    ...
    edgesToAdd.forEach(edge => {
      ...
      if ((0, collapsibility_1.isCollapsed)(source)) nodesToCollapse.set(edge.source, source);
    });
