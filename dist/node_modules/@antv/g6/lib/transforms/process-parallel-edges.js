  ...
  constructor(context, options) {
    ...
    this.getAffectedParallelEdges = input => {
      ...
      const pushParallelEdges = edge => {
        ...
        const validEdgeData = model.getEdgeData().filter(edge => !removedEdgeIds.has((0, id_1.idOf)(edge))).map(edge => (0, get_edge_actual_ends_1.getEdgeEndsContext)(model, edge));
    ...
    this.applyBundlingStyle = (input, edges, distance) => {
      const {
        edgeMap,
        reverses
      } = (0, exports.groupByEndpoints)(edges);
    ...
    this.applyMergingStyle = (input, edges) => {
      ...
      edgeMap.forEach(edges => {
        ...
        const mergedStyle = edges.map(({
          ...
          if ((0, util_1.isBoolean)(startArrow)) newStyle[start] = startArrow;
          if ((0, util_1.isBoolean)(endArrow)) newStyle[end] = endArrow;
          return newStyle;
        ...
        edges.forEach((edge, i, edges) => {
          ...
          const parsedStyle = Object.assign({}, (0, util_1.isFunction)(this.options.style) ? this.options.style(edges) : this.options.style, {
            childrenData: edges
          });
