...
class CollapseExpandCombo extends base_transform_1.BaseTransform {
  beforeDraw(input, context) {
    ...
    if (!this.context.model.model.hasTreeStructure(constants_1.COMBO_KEY)) return input;
    const {
      model
    } = this.context;
    const {
    ...
    while (combos.length) {
      ...
      if ((0, collapsibility_1.isCollapsed)(combo)) {
        ...
        const {
          internal,
          external
        } = (0, edge_1.getSubgraphRelatedEdges)(descendantIds, id => model.getRelatedEdgesData(id));
        ...
        const {
          edges
        } = (0, edge_1.getSubgraphRelatedEdges)(childrenIds, id => model.getRelatedEdgesData(id));
        [...children, ...edges].forEach(descendant => {
          ...
          if (element) (0, utils_1.reassignTo)(input, 'update', elementType, descendant);else (0, utils_1.reassignTo)(input, 'add', elementType, descendant);
