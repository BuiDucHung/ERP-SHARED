...
class BaseCombo extends nodes_1.BaseNode {
  ...
  getCollapsedMarkerStyle(attributes) {
    ...
    const _a = (0, prefix_1.subStyleProps)(this.getGraphicStyle(attributes), 'collapsedMarker'),
      {
        type
      } = _a,
  ...
  getCollapsedMarkerText(type, attributes) {
    ...
    if (type === 'node-count') return model.getDescendantsData(this.id).filter(datum => model.getElementType((0, id_1.idOf)(datum)) === 'node').length.toString();
    if ((0, util_1.isFunction)(type)) return type(childrenData);
    return '';
  }
