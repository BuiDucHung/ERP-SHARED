...
class BaseEdge extends base_element_1.BaseElement {
  ...
  getKeyStyle(attributes) {
    ...
    g_1.Path.PARSED_STYLE_LIST.forEach(key => {
  ...
  getLoopPath(attributes) {
    ...
    const {
      ...
      clockwise,
      dist = defaultDist
    } = (0, prefix_1.subStyleProps)(this.getGraphicStyle(attributes), 'loop');
  ...
  getEndpoints(attributes, optimize = true, controlPoints = []) {
    ...
    if (!optimize) {
      const sourcePoint = sourcePort ? (0, element_1.getPortPosition)(sourcePort) : sourceNode.getCenter();
      const targetPoint = targetPort ? (0, element_1.getPortPosition)(targetPort) : targetNode.getCenter();
      return [sourcePoint, targetPoint];
    }
    const _controlPoints = typeof controlPoints === 'function' ? controlPoints() : controlPoints;
    const sourcePoint = (0, element_1.getConnectionPoint)(sourcePort || sourceNode, _controlPoints[0] || targetPort || targetNode);
    const targetPoint = (0, element_1.getConnectionPoint)(targetPort || targetNode, _controlPoints[_controlPoints.length - 1] || sourcePort || sourceNode);
    return [sourcePoint, targetPoint];
  }
  ...
  getBadgeStyle(attributes) {
    ...
    const _a = (0, prefix_1.subStyleProps)(attributes, 'badge'),
      {
        offsetX,
        offsetY,
        placement
  ...
  getArrowStyle(attributes, isStart) {
    ...
    const _a = (0, prefix_1.subStyleProps)(this.getGraphicStyle(attributes), arrowType),
      {
        size,
        type
      } = _a,
    ...
    return Object.assign((0, util_1.pick)(keyStyle, ['stroke', 'strokeOpacity', 'fillOpacity']), {
      width,
      height
    }, Object.assign({}, d && {
      d,
  ...
  drawHaloShape(attributes, container) {
    ...
    this.upsert('halo', g_1.Path, style, container);
  }
  drawBadgeShape(attributes, container) {
    const style = this.getBadgeStyle(attributes);
    this.upsert('badge', shapes_1.Badge, style, container);
  }
  ...
  drawKeyShape(attributes, container) {
    ...
    return this.upsert('key', g_1.Path, style, container);
  }
