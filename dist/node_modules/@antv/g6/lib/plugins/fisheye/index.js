...
class Fisheye extends base_plugin_1.BasePlugin {
  constructor(context, options) {
    ...
    this.renderLens = origin => {
      ...
      if (!this.isLensOn) {
        this.lens = new elements_1.Circle({
          style
        });
    ...
    this.updateStyle = (magnifiedStyleMap, originStyleMap) => {
      ...
      const {
        ...
        exit,
        keep
      } = (0, diff_1.arrayDiff)(Array.from(this.prevMagnifiedStyleMap.keys()), Array.from(magnifiedStyleMap.keys()), d => d);
    ...
    this.isWheelValid = event => {
      ...
      const origin = this.lens.getCenter();
      if ((0, vector_1.distance)(scaleOrigin, origin) > this.r) return false;
      return true;
    };
    ...
    this.isDragValid = event => {
      ...
      const origin = this.lens.getCenter();
      if ((0, vector_1.distance)(dragOrigin, origin) > this.r) return false;
      return true;
    };
  ...
  bindEvents() {
    ...
    if (scaleRBy === 'wheel' || scaleDBy === 'wheel') {
      ...
      (_a = this.graphDom) === null || _a === void 0 ? void 0 : _a.addEventListener(constants_1.CommonEvent.WHEEL, wheelFunc, {
        passive: false
      });
    }
  }
