...
class BubbleSets extends base_plugin_1.BasePlugin {
  constructor(context, options) {
    ...
    this.getPath = forceUpdateId => {
      ...
      if (!forceUpdateId && this.path && (0, util_1.isEqual)(currMembers, prevMembers) && (0, util_1.isEqual)(currAvoidMembers, prevAvoidMembers)) {
        return this.path;
      }
      const {
        enter: membersToEnter = [],
        exit: membersToExit = []
      } = (0, diff_1.arrayDiff)(prevMembers, currMembers, d => d);
      const {
        enter: avoidMembersToEnter = [],
        exit: avoidMembersToExit = []
      } = (0, diff_1.arrayDiff)(prevAvoidMembers, currAvoidMembers, d => d);
      if (forceUpdateId) {
...
BubbleSets.defaultOptions = Object.assign({
  ...
  stroke: 'blue',
  strokeOpacity: 0.2
}, bubblesets_js_1.defaultOptions);
/**
...
const convertToLine = (graph, ids) => {
  ...
  return idArr.map(id => {
    ...
    return bubblesets_js_1.Line.from({
      x1: source[0],
      y1: source[1],
      x2: target[0],
      y2: target[1]
