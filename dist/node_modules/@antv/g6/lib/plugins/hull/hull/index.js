...
const grid_handle_1 = require("./grid_handle");
const monotone_convex_hull_2d_1 = require("./monotone-convex-hull-2d");
const robust_segment_intersect_1 = require("./robust-segment-intersect");
...
function _intersect(segment, pointset) {
  for (let i = 0; i < pointset.length - 1; i++) {
    ...
    if ((0, robust_segment_intersect_1.segmentsIntersect)(segment[0], segment[1], seg[0], seg[1])) {
      return true;
    }
  }
  return false;
...
function hull(pointset, concavity, format) {
  ...
  const points = _filterDuplicates(_sortByX(format_1.formatUtil.toXy(pointset, format)));
  if (points.length < 4) {
    const concave = points.concat([points[0]]);
    return format ? format_1.formatUtil.fromXy(concave, format) : concave;
  }
  ...
  const convex = (0, monotone_convex_hull_2d_1.monotoneConvexHull2D)(points).reverse().map(idx => points[idx]); // ccw -> cw, indices -> points
  ...
  const concave = _concave(convex, Math.pow(maxEdgeLen, 2), maxSearchArea, (0, grid_handle_1.grid)(innerPoints, cellSize), new Set());
  return format ? format_1.formatUtil.fromXy(concave, format) : concave;
}
