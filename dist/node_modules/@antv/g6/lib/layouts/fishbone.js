 ...
 */
class FishboneLayout extends base_layout_1.BaseLayout {
  constructor() {
    super(...arguments);
    this.id = 'fishbone';
  }
  getRoot() {
    ...
    if ((0, util_1.isEmpty)(roots) || roots.length > 2) return;
    return roots[0];
  }
  formatSize(nodeSize) {
    const nodeSizeFunc = typeof nodeSize === 'function' ? nodeSize : () => nodeSize;
    return node => (0, size_1.parseSize)(nodeSizeFunc(node));
  }
  doLayout(root, options) {
    ...
    const calculateX = (0, util_1.memoize)(node => {
    ...
    const calculateY = (0, util_1.memoize)(node => {
      if (isRoot(node)) return height / 2;
      if (!isAtEvenDepth(node)) {
        // If the node has no children, calculate Y based on the parent
        if ((0, util_1.isEmpty)(node.children)) return getParentY(node.id) + vGap;
        ...
        if ((0, util_1.isEmpty)(lastChild.children)) return calculateY(lastChild) + vGap;
        ...
        if ((0, util_1.isEmpty)(prevSibling.children)) return calculateY(prevSibling) + vGap;
  ...
  placeAlterative(result, root) {
    ...
    const shouldFlip = nodeId => {
      ...
      if ((0, util_1.isEmpty)(ancestors)) return false;
