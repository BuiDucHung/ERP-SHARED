...
class ElementController {
  ...
  initContainer() {
    if (!this.container || this.container.destroyed) {
      ...
      this.container = canvas.appendChild(new g_1.Group({
        className: 'elements'
      }));
    }
  }
  ...
  getElementType(elementType, datum) {
    ...
    const userDefinedType = (0, data_1.isOverridable)(datum) ? ((_a = options[elementType]) === null || _a === void 0 ? void 0 : _a.type) || datum.type : datum.type;
    if (!userDefinedType) {
  ...
  getElementComputedStyle(elementType, datum) {
    ...
    const style = (0, data_1.isOverridable)(datum) ? Object.assign({}, themeStyle, paletteStyle, dataStyle, defaultStyle, themeStateStyle, stateStyle) : Object.assign({}, dataStyle);
  ...
  computeChangesAndDrawData(context) {
    ...
    const {
      ...
      ComboUpdated = [],
      ComboRemoved = []
    } = (0, util_1.groupBy)(tasks, change => change.type);
    const moveToAddedIfUnrendered = (updated, added) => {
      const keptUpdates = [];
      updated.forEach(change => {
        const id = (0, id_1.idOf)(change.value);
  ...
  createElement(elementType, datum, context) {
    ...
    if (!Ctor) return print_1.print.warn(`The element ${type} of ${elementType} is not registered.`);
  ...
  updateElement(elementType, datum, context) {
    ...
    (_a = this.context.animation) === null || _a === void 0 ? void 0 : _a.add({
      ...
      before: () => {
        ...
        if (stage !== 'collapse') (0, element_2.updateStyle)(element, style);
        if (stage === 'visibility') {
          ...
          if (!(0, cache_1.hasCachedStyle)(element, 'opacity')) (0, cache_1.cacheStyle)(element, 'opacity');
          this.visibilityCache.set(element, exactStage === 'show' ? 'visible' : 'hidden');
          if (exactStage === 'show') (0, visibility_1.setVisibility)(element, 'visible');
        }
      },
      after: () => {
        var _a;
        const element = this.elementMap[id];
        if (stage === 'collapse') (0, element_2.updateStyle)(element, style);
        if (exactStage === 'hide') (0, visibility_1.setVisibility)(element, this.visibilityCache.get(element));
  ...
  markDestroyElement(data) {
    Object.values(data.remove).forEach(elementData => {
      elementData.forEach(datum => {
        const id = (0, id_1.idOf)(datum);
        const element = this.getElement(id);
        if (element) (0, element_2.markToBeDestroyed)(element);
      });
    });
  }
    ...
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const forcePreLayout = opts => {
        if (Array.isArray(opts)) {
          return opts.map(o => Object.assign(Object.assign({}, o), {
            preLayout: true
          }));
        }
      ...
      const layoutResult = yield layout.simulate(layoutOptions ? forcePreLayout(layoutOptions) : undefined);
  ...
  collapseCombo(id, animation) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      if (model.getAncestorsData(id, constants_1.COMBO_KEY).some(datum => (0, collapsibility_1.isCollapsed)(datum))) return;
