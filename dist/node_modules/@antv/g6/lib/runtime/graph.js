...
class Graph extends event_emitter_1.default {
  ...
  initCanvas() {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      if (container instanceof canvas_1.Canvas) {
        ...
        this.emit(constants_1.GraphEvent.BEFORE_CANVAS_INIT, {
          container: $container,
          width,
          height
        });
        ...
        yield canvas.ready;
        this.emit(constants_1.GraphEvent.AFTER_CANVAS_INIT, {
          canvas
        });
      }
    });
  ...
  updateCanvas(options) {
    ...
    if (!canvas) return;
    if (renderer) {
      this.emit(constants_1.GraphEvent.BEFORE_RENDERER_CHANGE, {
        renderer: this.options.renderer
      });
      canvas.setRenderer(renderer);
      this.emit(constants_1.GraphEvent.AFTER_RENDERER_CHANGE, {
        renderer
      });
    }
    if (cursor) canvas.setCursor(cursor);
    if ((0, util_1.isNumber)(width) || (0, util_1.isNumber)(height)) this.setSize((_a = width !== null && width !== void 0 ? width : this.options.width) !== null && _a !== void 0 ? _a : 0, (_b = height !== null && height !== void 0 ? height : this.options.height) !== null && _b !== void 0 ? _b : 0);
  }
  initRuntime() {
    this.context.options = this.options;
    if (!this.context.batch) this.context.batch = new batch_1.BatchController(this.context);
    if (!this.context.plugin) this.context.plugin = new plugin_1.PluginController(this.context);
    if (!this.context.viewport) this.context.viewport = new viewport_1.ViewportController(this.context);
    if (!this.context.transform) this.context.transform = new transform_1.TransformController(this.context);
    if (!this.context.element) this.context.element = new element_1.ElementController(this.context);
    if (!this.context.animation) this.context.animation = new animation_1.Animation(this.context);
    if (!this.context.layout) this.context.layout = new layout_2.LayoutController(this.context);
    if (!this.context.behavior) this.context.behavior = new behavior_1.BehaviorController(this.context);
  }
  prepare() {
    return __awaiter(this, void 0, void 0, function* () {
  ...
  resize(width, height) {
    ...
    if ((0, util_1.isEqual)(specificSize, canvasSize)) return;
    (0, event_1.emit)(this, new event_1.GraphLifeCycleEvent(constants_1.GraphEvent.BEFORE_SIZE_CHANGE, {
      size: specificSize
    }));
    this.context.canvas.resize(...specificSize);
    (0, event_1.emit)(this, new event_1.GraphLifeCycleEvent(constants_1.GraphEvent.AFTER_SIZE_CHANGE, {
      size: specificSize
    }));
  }
  /**
  ...
  translateElementBy(args1_1, args2_1) {
    return __awaiter(this, arguments, void 0, function* (args1, args2, args3 = true) {
      var _a, _b;
      const [config, animation] = (0, util_1.isObject)(args1) ? [args1, (_a = args2) !== null && _a !== void 0 ? _a : true] : [{
        [args1]: args2
      }, args3];
  ...
  setElementZIndex(args1, args2) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const config = (0, util_1.isObject)(args1) ? args1 : {
        [args1]: args2
      };
      Object.entries(config).forEach(([id, value]) => {
        const elementType = this.getElementType(id);
    ...
    return __awaiter(this, void 0, void 0, function* () {
      ...
      ids.map(_id => {
        ...
        if (elementType === 'combo') {
          ...
          const {
            internal
          } = (0, edge_1.getSubgraphRelatedEdges)(descendants.map(id_1.idOf), id => model.getRelatedEdgesData(id));
  ...
  collapseElement(id_2) {
    return __awaiter(this, arguments, void 0, function* (id, options = true) {
      ...
      if ((0, collapsibility_1.isCollapsed)(model.getNodeLikeData([id])[0])) return;
