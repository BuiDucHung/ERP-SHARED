...
function getPortXYByPlacement(bbox, placement, portMap = PORT_MAP, isRelative = true) {
  ...
  const p = (0, util_1.isString)(placement) ? (0, util_1.get)(portMap, placement.toLocaleLowerCase(), DEFAULT) : placement;
  if (!isRelative && (0, util_1.isString)(placement)) return p;
  const [x, y] = p || DEFAULT;
...
function getPortConnectionPoint(port, opposite) {
  ...
  if ((0, is_1.isPoint)(port)) return port;
  ...
  const oppositePosition = (0, is_1.isPoint)(opposite) ? opposite : isNode(opposite) ? opposite.getCenter() : opposite.getPosition();
...
function setAttributes(element, style) {
  ...
  if ((0, util_1.isNumber)(zIndex)) element.setAttribute('zIndex', zIndex);
