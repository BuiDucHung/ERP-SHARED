...
exports.createAnimationsProxy = createAnimationsProxy;
exports.preprocessKeyframes = preprocessKeyframes;
exports.inferDefaultValue = inferDefaultValue;
exports.getAnimationOptions = getAnimationOptions;
exports.getElementAnimationOptions = getElementAnimationOptions;
...
function preprocessKeyframes(keyframes) {
  ...
  Object.entries(propertyIndexedKeyframes).forEach(([key, values]) => {
    ...
    values.some(value => (0, util_1.isNil)(value)) ||
    // 属性值必须不完全一致 / property value must not be exactly the same
    // 属性值可以是保留属性 / property value can be the reserved property
    values.every(value => !['sourceNode', 'targetNode', 'childrenNode'].includes(key) && (0, util_1.isEqual)(value, values[0]))) {
...
function getAnimationOptions(options, localAnimation) {
  ...
  const effectTiming = Object.assign({}, constants_1.DEFAULT_ANIMATION_OPTIONS);
  if ((0, util_1.isObject)(animation)) Object.assign(effectTiming, animation);
  if ((0, util_1.isObject)(localAnimation)) Object.assign(effectTiming, localAnimation);
  return effectTiming;
}
/**
 * <zh/> 获取动画配置
...
function animationOf(options) {
  if (typeof options === 'string') {
    const animation = (0, get_1.getExtension)(constants_1.ExtensionCategory.ANIMATION, options);
    if (animation) return animation;
    print_1.print.warn(`The animation of ${options} is not registered.`);
    return [];
  }
  return options;
}
...
function getElementAnimationOptions(options, elementType, stage, localAnimation) {
  ...
  const themeElementAnimation = (_b = (0, theme_1.themeOf)(options)[elementType]) === null || _b === void 0 ? void 0 : _b.animation;
  const combine = (_ = []) => animationOf(_).map(animation => Object.assign(Object.assign(Object.assign(Object.assign({}, constants_1.DEFAULT_ELEMENTS_ANIMATION_OPTIONS), (0, util_1.isObject)(globalAnimation) && globalAnimation), animation), (0, util_1.isObject)(localAnimation) && localAnimation));
