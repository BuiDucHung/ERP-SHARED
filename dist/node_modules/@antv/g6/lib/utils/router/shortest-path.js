...
const defaultCfg = {
  ...
  distFunc: vector_1.manhattanDistance
};
...
function alignToGrid(p, gridSize) {
  ...
  if ((0, util_1.isNumber)(p)) return align(p);
  return p.map(align);
}
/**
...
const getObstacleMap = (nodes, options) => {
  ...
  nodes.forEach(item => {
    ...
    const bbox = (0, bbox_1.getExpandedBBox)(item.getRenderBounds(), offset);
...
const getBoxPoints = (point, node, directions, options) => {
  ...
  const expandedBBox = (0, bbox_1.getExpandedBBox)(node.getRenderBounds(), offset);
  const points = Object.keys(directionMap).reduce((res, directionKey) => {
    if (directions.includes(directionKey)) {
      ...
      const [width, height] = (0, bbox_1.getBBoxSize)(expandedBBox);
      const otherPoint = [point[0] + direction.stepX * width, point[1] + direction.stepY * height];
      const segments = (0, bbox_1.getBBoxSegments)(expandedBBox);
      for (let i = 0; i < segments.length; i++) {
        const intersectP = (0, line_1.getLinesIntersection)([point, otherPoint], segments[i]);
        if (intersectP && (0, bbox_1.isPointOnBBoxBoundary)(intersectP, expandedBBox)) {
          res.push(intersectP);
        }
      }
    }
  ...
  if (!(0, bbox_1.isPointInBBox)(point, expandedBBox)) {
    points.push(point);
  }
...
const getControlPoints = (current, cameFrom, scaleStartPoint, endPoint, startPoints, scaleEndPoint, gridSize) => {
  ...
  const startPoint = getNearestPoint(realStartPoints, pointZero, vector_1.manhattanDistance);
...
function aStarSearch(sourceNode, targetNode, nodes, config) {
  const startPoint = (0, vector_1.toVector2)(sourceNode.getCenter());
  const endPoint = (0, vector_1.toVector2)(targetNode.getCenter());
  ...
  while (Object.keys(openList).length > 0 && remainLoops > 0) {
    ...
    for (const dir of Object.values(options.directionMap)) {
      const neighbor = (0, vector_1.add)(current, [dir.stepX, dir.stepY]);
