...
function assignColorByPalette(data, palette) {
  ...
  const assignColor = args => {
    const palette = typeof colorPalette === 'string' ? (0, get_1.getExtension)('palette', colorPalette) : colorPalette;
  ...
  if (type === 'group') {
    ...
    const [min, max] = data.reduce(([min, max], datum) => {
      ...
      if (typeof value !== 'number') throw new Error((0, print_1.format)(`Palette field ${field} is not a number`));
